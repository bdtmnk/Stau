...
void PlotLimit_LSP150_BDTmutau_stau100_LSP1My_15_15_comb_35(TString fileList = "list_stau-stau_left_LSP150_BDTmutau_stau100_LSP1My_15_15_comb_35invfb",
	       ...
	       TString massLSP = "150",
	       TString massPart = "m_{#tilde{#tau_{L}}} (GeV)",
	       bool blindData = false){

  // char * filelist - List of files (output RooT files
  //                   produced by 'combine -M AsymptoticLimits')
...
//  fileName = argv[1];
  ...
  frame->GetYaxis()->SetTitle("95% CL limit on #sigma/#sigma^{theo}");
  ...
  TLegend * leg = new TLegend(0.6,0.65,0.75,0.80);
  leg->SetFillColor(0);
  ...
  line->SetLineStyle(2);
  line->Draw();
  TString legLSP = "m_{#tilde{#chi}_{1}^{0}} = " + massLSP + " GeV";
  TLatex * lsp = new TLatex(0.20,0.75,legLSP);
  lsp->SetNDC();
 ...
 // CMS_lumi(pad,4,11); 
 //DrawCMSLogo(pad, "CMS", "Preliminary", 11, 0.025, 0.015, 1.);
  CMS_lumi(canv,4,11); 
  //pad->RedrawAxis();
