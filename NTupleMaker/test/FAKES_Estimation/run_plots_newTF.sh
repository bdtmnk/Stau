...
######### C region non normal Tau
echo $line , $file, $fileB, $channel, $channel2

######### signal

#if [[ $file == *"C_OS"*  ]] && [[ $file == *"stau"*  || $file == *"C1"*  || $file == *"Chi"* ]];then
if [[ ! -f $dir/plots_$channel/${fileB}_C.root ]] ; then
#if true ; then
...
cp $dir/b_analyzer_h analyzer.h
cp $dir/b_analyzer${channel}TF_C analyzer.C
...
isA=[! -f $dir/plots_$channel/${fileB}_D.root]s
...
echo the filein : $file , the fileout : ${fileB}_D.root , region loose no tight Tau
sed -i 's/FILEIN/'$file'/g' analyzer*
sed -i 's/LEPTONHERE/false/g' analyzer.C
sed -i 's/SIGNHERE/OS/g' analyzer.C
sed -i 's/CHANNELHERE/'$channel2'/g' analyzer*
