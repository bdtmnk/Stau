...
/*
	variables.push_back("met_MT2lester_16");
	variables.push_back("met_DZeta_16");
	variables.push_back("MT2lester_DZeta_16");
	...
	variables.push_back("met_MT2lester_DZeta1D_16");
	variables.push_back("met_MT2lester_DZeta1D_17");

	variables.push_back("met_MT2lester_DZeta0J1D_16");
	variables.push_back("met_MT2lester_DZeta0J1D_17");

	variables.push_back("met_MT2lester_DZeta1J1D_16");
	variables.push_back("met_MT2lester_DZeta1J1D_17");
	
	variables.push_back("met_MT2lester_DZeta01J1D_16");
...
//	}
	...
	TH1D* allbkg, *htt,*hstop,*hwj,*hdyj,*hrare,*hdib,*hqcd,*httx, *hrest;
	...
	int count=0;
		...
		if (std::string::npos != n.find(variable) && obj)
		{
			...
			for (int sc = 0 ; sc<scales.size();sc++){
				...
				TString smFilename = "Templates_"+variable+"_"+lumistring+"_mt_C1N2.root";
				//TString smFilename = "Templates_mt.root";
				TFile *smFile = TFile::Open (smFilename, "recreate");
				
				TH1D* all_;
						
				all_ = (TH1D*) allbkg->Clone();
	...
	if(b_scale)		allbkg->Scale(scale);
				...
				for (int nb=1;nb<all_->GetNbinsX()+1;++nb){

				float bc_ = all_->GetBinContent(nb);

				if (bc_>100) {
				htt->SetBinContent(nb,0);
				hwj->SetBinContent(nb,0);
				hdyj->SetBinContent(nb,0);
				hstop->SetBinContent(nb,0);
				hdib->SetBinContent(nb,0);
				hqcd->SetBinContent(nb,0);
				httx->SetBinContent(nb,0);
	...
	if(b_scale)			hh[ij+2]->Scale(scale);
				...
				for (int nb=1;nb<all_->GetNbinsX()+1;++nb){
				float bc_ = all_->GetBinContent(nb);
				if (bc_>100) histbkg->SetBinContent(nb,0);
				}
					histbkg->Write();
...
ModifyHistErr (TH1D* &h, int cl_ ,string & title_,bool Up)
{


	float bc_ = h->GetBinContent(cl_);
	float er_ = h->GetBinError(cl_);
	char c_[10]
	if (Up)	 {h->SetBinContent(cl_,bc_+er_);
	if (!Up) h->SetBinContent(cl_,bc_-er_);
