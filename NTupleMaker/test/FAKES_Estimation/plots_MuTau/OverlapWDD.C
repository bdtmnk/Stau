...
void Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs){
	...
	path.Remove (0, 2);
	float Lumi= 58820.12 -20267.373725;
//	Lumi = 5125.664983 + 3509.331968 + 11632.376774;
	...
	//cout<< " for first source file, there where "<<nGen<<"
	//events with xsec "<<xsec<<" weight "<<lumiweights[0]<<endl;
	//" weight "<<float(xsec*Lumi/nGen)<<"  norm "<<norm<<endl;
...
//
	    ...
	    //if (string::npos != nn.find("Delta") ) continue;
                if (string::npos == nn.find("_15") && string::npos == nn.find("histWeightsH") && string::npos == nn.find("_16")) flcont=false;
//	    if (string::npos == nn.find("_15") && string::npos == nn.find("_16")) flcont=false;
//        if ( string::npos == nn.find("nJet") || string::npos == nn.find("_1") && string::npos == nn.find("_2")  && string::npos == nn.find("_13") && string::npos == nn.find("_16") && string::npos == nn.find("_17") && string::npos == nn.find("_18")  && string::npos == nn.find("_19")) flcont=false;
        //if (string::npos == nn.find("_12") && string::npos == nn.find("_10"))  flcont=false;
		if (!flcont) continue;
		...
		if (obj->IsA ()->InheritsFrom ("TH3") )Суекуишут.7.
		 continue;
		if (obj->IsA ()->InheritsFrom ("TH1D") ) {
			...
			cout<<"File name: "<<fname<<endl;
				  ...
				  TH1D  *eventCount;
				  eventCount = (TH1D*)nextsource->Get(Channel+"/histWeightsH"); 
				  float nGenn = eventCount->GetSumOfWeights();

				  //if  ( std::string::npos == fname.find("TTT") && std::string::npos == fname.find("DataDriven")) eventCountt = (TH1D*)nextsource->Get(Channel+"/histWeightsH");
				  //if ( std::string::npos != fname.find("TTT") ) eventCountt = (TH1D*)nextsource->Get(Channel+"/histTopPt");
				  float xsecc = xsecs[cl];
   //               		  cout <<xsecc <<"  "<< Lumi << "   "<<nGenn <<endl;
        ...
        cout<<" VV + TTX  "<<hdib->GetSumOfWeights()+httx->GetSumOfWeights()+hwj->GetSumOfWeights()<<" TT+ST "<<htt->GetSumOfWeights()+hstop->GetSumOfWeights()<<" WJ "<<hwj->GetSumOfWeights()<<" DY "<<hdyj->GetSumOfWeights()+hztt->GetSumOfWeights()<<" TTX "<<httx->GetSumOfWeights()<<" QCD "<<hqcd->GetSumOfWeights()<<endl;
		...
		if (obj->IsA ()->InheritsFrom ("TH1")) {
			...
			//<<" Found it here  "<<nan_<<"   "<<key->GetName()<<endl;
	...
	if (drawData)	pad2->Draw();
			...
			if (cl=1){
				...
				legend_c1->AddEntry(hh[2], "#tilde{#tau}_{L}(90,1)", "LEP");
				legend_c1->AddEntry(hh[3], "#tilde{#tau}_{L}(100,1)", "LEP");
    ...
    void ModifyHist (TH1D* &h, int cl_ ,float & lumi,float & weight,string & title_, bool norm_=false,float XSec=1.){
			...
			if (std::string::npos != title_.find("Data") || std::string::npos != title_.find("Single")
			    || std::string::npos != title_.find("MuonEG")  )  {
			...
			if ( std::string::npos != title_.find("tau") || std::string::npos != title_.find("C1")
			    || std::string::npos != title_.find("SMS")){
			...
			if (std::string::npos != title_.find("TTT") || std::string::npos != title_.find("TTPow")){col= mycolortt;}
			...
			if ( std::string::npos != title_.find("WW") ) {col=mycolorww;}


			if (std::string::npos != title_.find("wJets")|| std::string::npos != title_.find("WJetsToLNu") || std::string::npos != title_.find("W1JetsToLNu") || std::string::npos != title_.find("W2JetsToLNu") 
			|| std::string::npos != title_.find("W3JetsToLNu") || std::string::npos != title_.find("W4JetsToLNu"))  { col=mycolorvv ; }

			if ( ( std::string::npos != title_.find("ZZ") ||  std::string::npos != title_.find("WZ") || std::string::npos != title_.find("WG") || std::string::npos != title_.find("ZG") ) &&  std::string::npos == title_.find("WW")) {col=mycolorvv;}
...
//			if (std::string::npos != title1.find("MCTb"))  h->GetXaxis()->SetTitle("M_{CTb} [GeV]");
			...
			if (std::string::npos != title1.find("BDTmutau_Stau100"))  {h->GetXaxis()->SetTitle("BDT (100,1)");}
			if (std::string::npos != title1.find("BDTmutau_Stau150"))  h->GetXaxis()->SetTitle("BDT (150,1)");
			if (std::string::npos != title1.find("BDTmutau_Stau200"))  h->GetXaxis()->SetTitle("BDT (200,1)");
			if (std::string::npos != title1.find("BDTmutau_Stau300"))  h->GetXaxis()->SetTitle("BDT (300,1)");
...
void OverFlow(TH1D *& h, int &last_bin)
    {
        ...
        float contlast = 0.;
        for (int b=last_bin; b <= nb+1; ++b) {

            contlast +=h->GetBinContent(b);
            h->SetBinContent(b,0.);
            h->SetBinError(b,0.);
