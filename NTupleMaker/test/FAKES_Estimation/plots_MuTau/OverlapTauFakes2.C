...
void OverlapTauFakes2()
{
	...
	while(std::getline(ifs, line)) // read one line from ifs
	{
		...
		titles.push_back(dataname+"_Nominal_D.root");
	...
	string fout = "Fakes_Nominal_A.root";
...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	while ((key = (TKey *) nextkey ())) {
        ...
        if (string::npos == nn.find("_15") && string::npos == nn.find("_14")  && string::npos == nn.find("_13") && string::npos == nn.find("_16") && string::npos == nn.find("histWeightsH") && string::npos == nn.find("_17") && string::npos == nn.find("_18")) flcont=false;
