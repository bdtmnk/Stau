...
void OverlapQCDUnclEnDown()
{
	...
	Float_t value=0;
	vector<float> xsecs_;
	ifstream ifs("QCDsamplesUnclEnDown");
	string line;
	...
	string fout = "QCD_UnclEnDown_B.root";
...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	path.Remove (0, 2);
				...
				TFile *foutA = TFile::Open ("QCD_UnclEnDown_A.root", "RECREATE");
				TFile *foutC = TFile::Open ("QCD_UnclEnDown_C.root", "RECREATE");
				TFile *foutD = TFile::Open ("QCD_UnclEnDown_D.root", "RECREATE");
	...
	ofstream tfile;
	TFile * filee = new TFile("QCD_UnclEnDown_B.root","update");
	filee->cd("mutau");
	//target->ls();
