...
_lt=`echo $line | cut -d '/' -f2`
	...
	echo $lt > list_$_lt
	ltt=`echo $_lt | awk -F ".root" '{print $1}'`
	for syst in $systematics
	do
	if [ ! -f ./plots_MuTau/${ltt}*.root  ] ;then
	echo $_lt
		ltt=`echo $_lt | awk -F ".root" '{print $1}'`
		if [[ true ]] ; then
		echo $ltt
		cat bss > job_MuTau_${channel}_${_lt}_$syst
		echo python /nfs/dust/cms/user/dydukhle/STAU/Analysis18/CMSSW_10_2_16/src/DesyTauAnalyses/NTupleMaker/test/FAKES_Estimation/run_plots_newDataDriven_new_MuTau_B.py list_$_lt $2 $syst >> job_MuTau_${channel}_${_lt}_$syst
