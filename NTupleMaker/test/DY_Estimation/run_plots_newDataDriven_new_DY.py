...
os.system("cd /nfs/dust/cms/user/dydukhle/STAU/Analysis18/CMSSW_10_2_16/src/DesyTauAnalyses/NTupleMaker/test/DY_Estimation/")#;eval `scramv1 runtime -sh` ;
dir="/nfs/dust/cms/user/dydukhle/STAU/Analysis18/CMSSW_10_2_16/src/DesyTauAnalyses/NTupleMaker/test/DY_Estimation/"
i=0
channel=arguments[2]
...
for i in range(len(content)):
    ...
    path = "dy_dir_{0}_{1}_{2}".format(fileB, channel, syst)
    print(file)
    try:
            os.makedirs(path)
    except Exception as e:pass#;print("Exception: ", e)
    try:
    	working_dir = "dy_dir_{0}_DY_{2}".format(fileB, channel, syst)
    ...
    if os.path.exists("./plots_DY/{0}_B.root".format(fileB)):
    	continue
    #os.mkdir(working_dir)
    #os.system("mkdir -f {0}".format(working_dir))
    os.system("cp -f {0}/analyzer_DY.h {0}/{1}/.".format(dir, working_dir))
    os.system("mv {0}/{1}/analyzer_DY.h {0}/{1}/analyzer.h".format(dir, working_dir))
    os.system("cp -f {0}/analyzer_DY.C {0}/{2}/.".format(dir, channel, working_dir))
    os.system("mv -f {0}/{2}/analyzer_DY.C {0}/{2}/analyzer.C".format(dir, channel, working_dir))
    os.system("cp -f {0}/runme.C {0}/{1}/.".format(dir,working_dir))
    os.system("cp -f {0}/plots_DY.h {0}/{1}/.".format(dir, working_dir))
    os.system("mv -f {0}/{1}/plots_DY.h {0}/{1}/plots.h".format(dir, working_dir))
    ...
    os.system("sed -i 's/REGION/{0}/g' analyzer*.*".format('SR', working_dir))
    ...
    os.system("mv {0}/{3}/plots.root {0}/plots_DY/{2}_B.root".format(dir, channel, fileB, working_dir))
