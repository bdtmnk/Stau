...
class analyzer {
 public :
  ...
  TBranch        *b_bjets_cleaned;   //!
  TBranch        *b_bjets_cleanedTight;   //!
  TBranch        *b_jet_e;   //!
  TBranch        *b_jet_px;   //!
  TBranch        *b_jet_py;   //!
  TBranch        *b_jet_pz;   //!
  ...
  virtual void SetAxisBins(TGraphAsymmErrors* graph);
  virtual bool check_SameBinning(TGraphAsymmErrors* graph1, TGraphAsymmErrors* graph2);
  virtual double get_EfficiencyData(double pt, double eta, TH1D *etaBinsH);
  virtual double get_EfficiencyMC(double pt, double eta, TH1D *etaBinsH);
  virtual std::string FindEtaLabel(double Eta, std::string Which,TH1D *etaBinsH);
  virtual int FindPtBin( std::map<std::string, TGraphAsymmErrors *> eff_map, std::string EtaLabel, double Pt);
  virtual double  get_ScaleFactor(double pt, double eta,TH1D *etaBinsH);
...
//cout<<""<<endl;
    ...
    if ( (Sel=="mutau" || Sel=="WJetsCRMuTau" )&& mIndex >-1 && tIndex>-1 ){
...
//cout<<" cat "<<category<<" catJet "<<categoryJet<<"   "<<isTight<<"  "<<etaBin<<endl;
    if ( mIndex >-1 && tIndex>-1 && etaBin>-1){
      ...
      float dPhiW = -99;
      if (JetsMV.size()>0 && muV.Pt()>0 && MetV.Pt()>0) 
      {
      	TLorentzVector Wb = muV  + MetV;
	      dPhiW = dPhiFrom2P( Wb.Px(), Wb.Py(), JetsMV.at(0).Px(),  JetsMV.at(0).Py() );
...
if (isLoose){
//	if (CutIndex>15)
//cout<<"  CutIndex  "<<CutIndex<<"  isL  "<<isLoose<<" isTight  "<<isTight<<" met "<<met<<" njets  "<<njets<<"  "<<dPhiW<<"  "<<MT<<endl;
...
if (dPhiW >-99)	    hDPhiCutL[CutIndex]->Fill(dPhiW, EvWeight);
	    ...
	    hMETCutL[CutIndex]->Fill(MetV.Pt(), EvWeight);
      	    hnJetsL[CutIndex]->Fill(njets,EvWeight);
	    hnbJetsL[CutIndex]->Fill(nbtag,EvWeight);

if (tauV.Pt()>0. && etaBin>-1)	    FakeRatePtIncLoose[etaBin][CutIndex]->Fill(tauV.Pt(),EvWeight);
}
...
if (tauV.Pt()>0. && etaBin>-1)	    FakeRatePtIncTight[etaBin][CutIndex]->Fill(tauV.Pt(),EvWeight);
...
//cout<<" insidecheck here  DEta " << fabs(deta)<<"   "<<fabs(detaM)<<" DPhi "<<dPhiD<<"   "<<endl;
      ...
      double MTDil = TMath::Sqrt(2*DiL.Pt()*met*(1-TMath::Cos(dPhi)));
      ...
      double MT = TMath::Sqrt(2*mu_pt[mIndex]*met*(1-TMath::Cos(dPhi)));
      ...
      double MTt = TMath::Sqrt(2*tauV.Pt()*met*(1-TMath::Cos(dPhi)));
...
/*
	...
	double dPhiMuMET=dPhiFrom2P( mu_px[mIndex], mu_py[mIndex], metx,  mety );
	double dPhiTauMET=dPhiFrom2P( tauV.Px(), tauV.Py(), metx,  mety );
        	...
        	hBDTmutau_stau100_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau150_LSP1My->GetMvaValue( *inputVec );
        	hBDTmutau_stau150_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau200_LSP1My->GetMvaValue( *inputVec );
        	hBDTmutau_stau200_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau300_LSP1My->GetMvaValue( *inputVec );
        	hBDTmutau_stau300_LSP1My[CutIndex]->Fill( retval, EvWeight );
*/
        	//double retval = BDTResponseMutau_stau150_LSP50My->GetMvaValue( *inputVec );
        	//hBDTmutau_stau150_LSP50My[CutIndex]->Fill( retval, EvWeight );
        	...
        	*/
        	...
        	/*
        	retval = BDTResponseMutau_stau100_LSP50My->GetMvaValue( *inputVec );
        	hBDTmutau_stau100_LSP50My[CutIndex]->Fill( retval, EvWeight );

        	retval = BDTResponseMutau_stau150_LSP50My->GetMvaValue( *inputVec );
        	hBDTmutau_stau150_LSP50My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau150_LSP1My->GetMvaValue( *inputVec );
        	hBDTmutau_stau150_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau150_LSP100My->GetMvaValue( *inputVec );
        	hBDTmutau_stau150_LSP100My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau200_LSP1My->GetMvaValue( *inputVec );
        	hBDTmutau_stau200_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau200_LSP50My->GetMvaValue( *inputVec );
        	hBDTmutau_stau200_LSP50My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau200_LSP100My->GetMvaValue( *inputVec );
        	hBDTmutau_stau200_LSP100My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau200_LSP150My->GetMvaValue( *inputVec );
        	hBDTmutau_stau200_LSP150My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseMutau_stau300_LSP1My->GetMvaValue( *inputVec );
        	hBDTmutau_stau300_LSP1My[CutIndex]->Fill( retval, EvWeight );
*/
    ...
    if ((Sel=="eltau" || Sel=="WJetsCRElTau")&& eIndex > -1 && tIndex>-1){
      ...
      double MT = TMath::Sqrt(2*elV.Pt()*met*(1-TMath::Cos(dPhi)));
      hMT[CutIndex]->Fill(MT,EvWeight);
...
/*
	double deta=deltaEta(elV.Px(),  elV.Py(),elV.Pz(), tauV.Px(),  tauV.Py(),tauV.Pz() );
	double dPhiMuMET=dPhiFrom2P( el_px[mIndex], el_py[mIndex], metx,  mety );
	double dPhiTauMET=dPhiFrom2P( tauV.Px(), tauV.Py(), metx,  mety );
			...
			double retval = BDTResponseEltau_stau100_LSP1My->GetMvaValue( *inputVec );
        	hBDTeltau_stau100_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau150_LSP1My->GetMvaValue( *inputVec );
        	hBDTeltau_stau150_LSP1My[CutIndex]->Fill( retval, EvWeight );        	
        	retval = BDTResponseEltau_stau200_LSP1My->GetMvaValue( *inputVec );
        	hBDTeltau_stau200_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau300_LSP1My->GetMvaValue( *inputVec );
        	hBDTeltau_stau300_LSP1My[CutIndex]->Fill( retval, EvWeight );        	
        	 */       	
        	/*
        	retval = BDTResponseEltau_stau100_LSP50My->GetMvaValue( *inputVec );
        	hBDTeltau_stau100_LSP50My[CutIndex]->Fill( retval, EvWeight );

        	retval = BDTResponseEltau_stau150_LSP50My->GetMvaValue( *inputVec );
        	hBDTeltau_stau150_LSP50My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau150_LSP1My->GetMvaValue( *inputVec );
        	hBDTeltau_stau150_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau150_LSP100My->GetMvaValue( *inputVec );
        	hBDTeltau_stau150_LSP100My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau200_LSP1My->GetMvaValue( *inputVec );
        	hBDTeltau_stau200_LSP1My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau200_LSP50My->GetMvaValue( *inputVec );
        	hBDTeltau_stau200_LSP50My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau200_LSP100My->GetMvaValue( *inputVec );
        	hBDTeltau_stau200_LSP100My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau200_LSP150My->GetMvaValue( *inputVec );
        	hBDTeltau_stau200_LSP150My[CutIndex]->Fill( retval, EvWeight );
        	retval = BDTResponseEltau_stau300_LSP1My->GetMvaValue( *inputVec );
        	hBDTeltau_stau300_LSP1My[CutIndex]->Fill( retval, EvWeight );
*/
    ...
    if (Sel=="muel" &&  eIndex >-1 &&  mIndex>-1){
      ...
      double MT = TMath::Sqrt(2*muV.Pt()*met*(1-TMath::Cos(dPhi)));


      hMTmu[CutIndex]->Fill(MT,EvWeight);
      hMTmuFineBin[CutIndex]->Fill(MT,EvWeight);
      dPhi=dPhiFrom2P( elV.Px(), elV.Py(), metx,  mety );
      double MTel = TMath::Sqrt(2*elV.Pt()*met*(1-TMath::Cos(dPhi)));
					
      dPhi=dPhiFrom2P( DiL.Px(), DiL.Py(), metx,  mety );
      double MTelmu = TMath::Sqrt(2*DiL.Pt()*met*(1-TMath::Cos(dPhi)));
...
hnJetByBin[CutIndex][nSR]->Fill(njets,EvWeight);

if (mIndex!=-1){
hMuptByBin[CutIndex][nSR]->Fill(mu_pt[mIndex],EvWeight);
hMuetaByBin[CutIndex][nSR]->Fill(mu_eta[mIndex],EvWeight);
}
if(tIndex!=-1){
hTauetaByBin[CutIndex][nSR]->Fill(ta_pt[tIndex],EvWeight);
hTauptByBin[CutIndex][nSR]->Fill(ta_eta[tIndex],EvWeight);
}

if (Mt2as>0){
hMt2lesterByBin[CutIndex][nSR]->Fill(Mt2as,EvWeight);
hMt2lesterFBByBin[CutIndex][nSR]->Fill(Mt2as,EvWeight);
}

if (dzeta>-500){
hDZetaByBin[CutIndex][nSR]->Fill(dzeta,EvWeight);
hDZetaFBByBin[CutIndex][nSR]->Fill(dzeta,EvWeight);
}

hMETByBin[CutIndex][nSR]->Fill(met,EvWeight);
hMETFBByBin[CutIndex][nSR]->Fill(met,EvWeight);

if (eIndex!=-1){
hElptByBin[CutIndex][nSR]->Fill(el_pt[eIndex],EvWeight);
hEletaByBin[CutIndex][nSR]->Fill(el_eta[eIndex],EvWeight);
...
*/
    ...
    if (eIndex != -1)
      {
	//hElEff[CutIndex]->Fill(elV.Pt(),fabs(elV.Eta()),EvWeight);
...
///////////////////COMMON MUON VARIABLES
    if (mIndex !=-1 )
      {

//	hMuEff[CutIndex]->Fill(muV.Pt(),fabs(muV.Eta()),EvWeight);
	...
	float btagged = 0.8484;//for jets with values above this are considered

    // Identified lepton
    LorentzVector lep(muV.Px(),muV.Py(),muV.Pz(),muV.E());

if (nbtag>0){
    // Jets, b-tagging, and jet-resolution
    for (int nj=0;nj<nbtag;nj++){

    int njj = bjets_cleaned[nj];
    LorentzVector j1(jet_px[njj],jet_py[njj],jet_pz[njj],jet_e[njj]);
    float b1 =  jet_btag[njj];
    bool id1 = true;
 
    hCSVv2[CutIndex]->Fill(jet_btag[njj],EvWeight);
if (njj==0)    hCSVv2_b1[CutIndex]->Fill(jet_btag[njj],EvWeight);
if (njj==1)    hCSVv2_b2[CutIndex]->Fill(jet_btag[njj],EvWeight);
//cout<<" btag  "<<nbtag<<"  njj "<<njj<<" b1 "<<b1<<" flavour "<<jet_flavour[njj]<<endl;
    jets.push_back(j1);
    btag.push_back(b1); 
    jid.push_back(id1); 
	}
    ...
    vector<int> jetIndexSortedBtag = JetUtil::JetIndexCSVsorted(btag, jets, jid, 20., 2.4, true);
...
//    cout<<"  "<<bjets.size()<<"  "<<additionaljets.size()<<"  "<<CutIndex<<"  "<<njets<<endl;
 ...
 //   float tmod1B = CalcTopness1BTag_(1,met,metphi,lep,bjets,additionaljets);

    // Print results.
//    if (CutIndex>100){
//    cout << "MT2W = " << MT2W << endl;
//    cout << "tmod = " << tmod << endl;
//    cout << "tmod1B = " << tmod1B << endl;
//    }

//    hTopMod[CutIndex]->Fill(tmod,EvWeight);
//    hTopMod1L[CutIndex]->Fill(tmod1B,EvWeight);
//    hMT2W[CutIndex]->Fill(MT2W,EvWeight);
//    hMT2WFB[CutIndex]->Fill(MT2W,EvWeight);
}
	if (nbtag>0 && bjets_cleaned[0]>-1) {
	TLorentzVector bV;
	int bjet_index = bjets_cleaned[0];

	bV.SetPxPyPzE(0.,0.,0.,0.);
	bV.SetPxPyPzE(jet_px[bjet_index], jet_py[bjet_index],jet_pz[bjet_index],jet_e[bjet_index]);
        double Dr =  muV.DeltaR( bV);
        hdR_BottomLepton[CutIndex]->Fill(Dr,EvWeight);

	//double cosT = cosAngle(bV,muV);
	TVector3 v1; v1.SetXYZ(bV.X(),bV.Y(),bV.Z());
	TVector3 v2; v2.SetXYZ(muV.X(),muV.Y(),muV.Z());
	TVector3 v3 = v1+v2;
	//double cosT = v3.CosTheta();
	double cosT2 = cosAngle(bV,muV);
//if (CutIndex==10 && bV.Pt()>0 && muV.Pt()>0)	cout<<"  cosTheta  "<<cosT<<"   "<<cosT2<<endl;
	hCosbl[CutIndex]->Fill(cosT2,EvWeight);
        double pl_4[4] = { muV.E(), muV.Px(), muV.Py(),muV.Pz()};
        double pb1_4[4] = {bV.E(), bV.Px(), bV.Py(), bV.Pz()};
        double pb2_4[4] = {tauV.E(), tauV.Px(),tauV.Py(),tauV.Pz()};
        //double pb2_4[4] = {0,0,0,0};
        double pmiss_4[3] = { 0., metx, mety};

	mt2bl_bisect::mt2bl mt2bl_event;
	mt2bl_event.set_momenta(pl_4,pb1_4,pb2_4,pmiss_4);

	TLorentzVector Mbl = muV + bV;

	double mt2bl =  mt2bl_event.get_mt2bl();
      	hMT2blmu[CutIndex]->Fill(mt2bl,EvWeight);
      	hMT2blmuFineBin[CutIndex]->Fill(mt2bl,EvWeight);

      	hInvMassblmu[CutIndex]->Fill(Mbl.M(),EvWeight);
      	hInvMassblmuFB[CutIndex]->Fill(Mbl.M(),EvWeight);

	//cout<<" mt2bl  "<<mt2bl<<endl;
      	...
      	double dPhi=dPhiFrom2P( mu_px[mIndex], mu_py[mIndex], metx,  mety );
	TLorentzVector Wv = muV + MetV;
	double Lp = (mu_pt[mIndex]/Wv.Pt())*TMath::Cos(dPhi);
      	hdPhiMuMET[CutIndex]->Fill(dPhi,EvWeight);
	
	hLpMu[CutIndex]->Fill(Lp,EvWeight);
        if (JetsV.size()>0){
        double RatioSums = (Wv.Pt() - JetsV.at(0).Pt() )/ (Wv.Pt() +JetsV.at(0).Pt() );
	hRatioSum[CutIndex]->Fill(RatioSums,EvWeight);
       }
	...
	sumMuonpT +=muV.Pt();
	hIsoMu[CutIndex]->Fill(mu_relIso[mIndex],EvWeight);
	hIsoMuFB[CutIndex]->Fill(mu_relIso[mIndex],EvWeight);
	...
	//cout<< "  taus "<<tIndex<<endl;
        ...
        htau_dxy[CutIndex]->Fill(mu_dxy[tIndex],EvWeight);
        htau_dz[CutIndex]->Fill(mu_dz[tIndex],EvWeight);

        double dPhiJ=dPhiFrom2P( tauV.Px(), tauV.Py(), metx,  mety );
	//cout << mIndex <<"  "<< ta_px[mIndex] <<"  "<< ta_py[mIndex] <<"  "<< metx <<"  "<< mety <<endl; 

	//cout <<"dPhiJ  "<< dPhiJ<<endl; 
	...
	hTauDecayModeAll[CutIndex]->Fill(genTauDecayMode2,EvWeight);
        ...
        hTauPtEta[CutIndex]->Fill(tauV.Pt(),fabs(tauV.Eta()),EvWeight);
      }
    ...
    if (JetsV.size()>1){
      ...
      double dPhiJ0=dPhiFrom2P( JetsV.at(0).Px(), JetsV.at(0).Py(), metx,  mety );
      double mindPhi = min (dPhiJ,dPhiJ0);
      hdPhiMinJMET[CutIndex]->Fill(mindPhi,EvWeight);
    ...
    if (JetsV.size()>2){
      ...
      TLorentzVector JetMass = JetsV.at(0) + JetsV.at(1) + JetsV.at(2);
      hTriJetMass_J0J1J2[CutIndex]->Fill(JetMass.M(),EvWeight);
    }
    if (JetsV.size()>3){
    ...
    hnBJetTight[CutIndex]->Fill(nbtagTight,EvWeight);

 if (JetsV.size()>0)   hHT[CutIndex]->Fill(sumpT,EvWeight);

    double HText =sumpT;
    if (Sel=="mutau" && mIndex !=-1)    HText += muV.Pt();
    ...
    if (Sel=="mutau" && mIndex !=-1 && JetsV.size()>0) {
      hRht[CutIndex]->Fill(muV.Pt()/HText,EvWeight);
      hPtOHT[CutIndex]->Fill(  muV.Pt()/sumpT,EvWeight);
    }
    ...
    if (Sel=="muel" && eIndex !=-1 && mIndex !=-1 && JetsV.size()>0) {
      hRht[CutIndex]->Fill( (elV.Pt()+muV.Pt())/HText,EvWeight);
      hPtOHT[CutIndex]->Fill(  (elV.Pt()+muV.Pt())/sumpT,EvWeight);
...
///////////////////////////////////////////////////////////////
  ...
  void FillHistsDiL(int CutIndex, Double_t EvWeight, TLorentzVector  LeptV1, TLorentzVector  LeptV2,  vector<TLorentzVector>  &JetsV, TLorentzVector  &MetV, double Chimass, double mintermediate, string & Sel, int  mIndex_1, int mIndex_2){
    ...
    if (Sel=="mumu"){
      ...
      double dPhi=dPhiFrom2P( DiL.Px(), DiL.Py(), metx,  mety );
      double MT = TMath::Sqrt(2*DiL.Pt()*met*(1-TMath::Cos(dPhi)));
      ...
      double MTlept1 = TMath::Sqrt(2*mu_pt[mIndex_1]*met*(1-TMath::Cos(dPhi)));
      ...
      double MTlept2 = TMath::Sqrt(2*mu_pt[mIndex_2]*met*(1-TMath::Cos(dPhi)));
...
if (pzetavis !=0) dzetaR = pzetamiss/pzetavis;
	...
	sumMuonpT +=LeptV1.Pt();
	hIsoLept1[CutIndex]->Fill(mu_relIso[mIndex],EvWeight);
...
analyzer::analyzer(TTree *tree) : fChain(0) 
{
  ...
  if (tree == 0) {
    ...
    f->GetObject("DIRHERE/T",tree);

#else // SINGLE_TREE

    // The following code should be used if you want this class to access a chain
    // of trees.
    TChain * chain = new TChain("DIRHERE/T","");
	
	string name= "FILEIN";
	

    //TString ChainName = "/nfs/dust/cms/user/dydukhle/CMSSW_9_4_0_patch1/src/DesyTauAnalyses/NTupleMaker/test/CHANNELHERE/FILEIN/DIRHERE/T";
    TString ChainName = "/nfs/dust/cms/user/dydukhle/STAU/Analysis18/CMSSW_10_2_16/src/DesyTauAnalyses/NTupleMaker/test/CHANNELHERE/FILEIN/DIRHERE/T";
    TString Tsystematic="SYSTEMATICHERE";
    
    if (string::npos == name.find("Single") && (Tsystematic == "JetEnUp" || Tsystematic == "JetEnDown" || Tsystematic == "UnclEnDown" || Tsystematic == "UnclEnDown" || Tsystematic == "TauEnUp" || Tsystematic == "TauEnDown" || Tsystematic == "ElEnUp" || Tsystematic == "ElEnDown" || Tsystematic == "MuEnUp" || Tsystematic == "MuEnDown" || Tsystematic == "BTagUp" || Tsystematic == "BTagDown")) 
//      ChainName ="/nfs/dust/cms/user/dydukhle/CMSSW_9_4_0_patch1/src/DesyTauAnalyses/NTupleMaker/test/CHANNELHERE_"+Tsystematic+"/FILEIN/DIRHERE/T";
	 ChainName = "/nfs/dust/cms/user/dydukhle/STAU/Analysis18/CMSSW_10_2_16/src/DesyTauAnalyses/NTupleMaker/test/CHANNELHERE/"+Tsystematic+"/FILEIN/DIRHERE/T"; 
...
void analyzer::Init(TTree *tree)
{
  ...
  fChain->SetBranchAddress("wPDFDown", &wPDFDown, &b_wPDFDown);
    fChain->SetBranchAddress("ta_IsoDPF0", &ta_IsoDPF0, &b_ta_IsoDPF0);
  ...
  fChain->SetBranchAddress("bjets_cleaned", &bjets_cleaned, &b_bjets_cleaned);
  fChain->SetBranchAddress("bjets_cleanedTight", &bjets_cleanedTight, &b_bjets_cleanedTight);
  ...
  fChain->SetBranchAddress("genLeptonMatchedPromptEl", &genLeptonMatchedPromptEl, &b_genLeptonMatchedPromptEl);
  fChain->SetBranchAddress("genLeptonMatchedPromptMu", &genLeptonMatchedPromptMu, &b_genLeptonMatchedPromptMu);
  fChain->SetBranchAddress("genLeptonMatchedPromptTau", &genLeptonMatchedPromptTau, &b_genLeptonMatchedPromptTau);
  ...
  fChain->SetBranchAddress("genMuMatchedHadrDecay", &genMuMatchedHadrDecay, &b_genMuMatchedHadrDecay);
  fChain->SetBranchAddress("genElMatchedHadrDecay", &genElMatchedHadrDecay, &b_genElMatchedHadrDecay);
  fChain->SetBranchAddress("genTauMatchedHadrDecay", &genTauMatchedHadrDecay, &b_genTauMatchedHadrDecay);
