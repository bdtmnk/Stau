...
TH1D* h1 = (TH1D*)in1->Get("mutau/"+Variable);
TH1D* h2 = (TH1D*)in2->Get("mutau/"+Variable);
TH1D* hw = (TH1D*)in1->Get("mutau/histWeightsH");
...
outUp->cd();
if (!(outUp->GetDirectory("mutau"))) outUp->mkdir("mutau");
outUp->cd("mutau");
hww->Write();
hUp->Write();
outUp->Close();
...
outDown->cd();
if (!(outDown->GetDirectory("mutau"))) outDown->mkdir("mutau");
outDown->cd("mutau");
hww->Write();
hDown->Write();
outDown->Close();
