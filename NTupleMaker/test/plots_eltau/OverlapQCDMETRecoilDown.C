...
void OverlapQCDMETRecoilDown()
{
	...
	Float_t value=0;
	vector<float> xsecs_;
	ifstream ifs("QCDsamplesMETRecoilDown");
	string line;
	...
	string fout = "QCD_METRecoilDown_B.root";
...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	path.Remove (0, 2);
				...
				TFile *foutA = TFile::Open ("QCD_METRecoilDown_A.root", "RECREATE");
				TFile *foutC = TFile::Open ("QCD_METRecoilDown_C.root", "RECREATE");
				TFile *foutD = TFile::Open ("QCD_METRecoilDown_D.root", "RECREATE");
	...
	while ((key = (TKey *) nextkey ())) {
		...
		c1->SetTitle(obj->GetName());
			...
			while (nextsource) {
				...
				nextsource->cd("eltau");
				TH1D* eventCountt ;
				  if  ( std::string::npos == fname.find("TT_TuneCUETP8M2T4_13TeV-powheg-pythia8")) eventCountt = (TH1D*)nextsource->Get("eltau/histWeightsH");
				  if ( std::string::npos != fname.find("TT_TuneCUETP8M2T4_13TeV-powheg-pythia8") ) eventCountt = (TH1D*)nextsource->Get("eltau/histTopPt");
	...
	ofstream tfile;
	TFile * filee = new TFile("QCD_METRecoilDown_B.root","update");
	filee->cd("eltau");
	//target->ls();
