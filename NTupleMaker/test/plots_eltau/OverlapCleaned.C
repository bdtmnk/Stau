...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles)
{
	...
	//since first source must be data, norm = 1;
	norm =1;
	...
	while ((key = (TKey *) nextkey ())) {
		...
		if (obj->IsA ()->InheritsFrom ("TH1D") ) {
			...
			while (nextsource) {
				...
				//if (std::string::npos != fname.find(tt)) xsecc = 831.76*0.9691;
	...
	//				cout<< " for "<<fname<<"  "<<count<<" file, there xsec is "<<xsecc<<" SuOfWeights "<<nGenn<<" Lumi "<<Lumi<<" lumiweight "<<lumiweights[cl-1]<<" weight "<<float(xsecc*Lumi/nGenn)<<"  norm "<<normm<<" cl "<<cl<<"  "<<h2->Integral()<<endl;
		...
		if (obj) {
			...
			TPad *pad1 = new TPad("pad1","pad1",0,0.25,1,1);//ratio
			...
			if (cl=1){
				...
				char lab_[100];
				legend_c1->AddEntry(hh[2], "#tau(50,1)", "LEP");
				legend_c1->AddEntry(hh[3], "#tau(100,1)", "LEP");
				legend_c1->AddEntry(hh[4], "#tau(150,1)", "LEP");
				legend_c1->AddEntry(hh[5], "#tau(200,1)", "LEP");
				legend_c1->AddEntry(hh[6], "#tau(250,1)", "LEP");
				legend_c1->AddEntry(hh[7], "#tau(300,1)", "LEP");
				legend_c1->AddEntry(hh[8], "#tau(350,1)", "LEP");
				legend_c1->AddEntry(hh[9], "#tau(400,1)", "LEP");
				legend_c1->AddEntry(hh[10], "#tau(450,1)", "LEP");
				legend_c1->AddEntry(hh[11], "#tau(500,1)", "LEP");
				...
				//cout<<" COLOR CODEEEEEEEEEEEEEEEEE  "<<cl<<endl;
				...
				v7->SetLineColor(kOrange-4);legend_c1->AddEntry(v7, "TTX/TG", "l");
	...
	*/
			string n = hh[1]->GetName();
				const char *s1;
				float scale = float(15000/Lumi);
			if (std::string::npos != n.find("hDZeta_19"))
			{            
				smFile->cd();
				allbkg = hsum;
				allbkg->SetMinimum(0.001);
				...
				htt->SetMinimum(0.001);
				hdyj->SetMinimum(0.001);
				hwj->SetMinimum(0.001);
				httx->SetMinimum(0.001);
				hstop->SetMinimum(0.001);
				hdib->SetMinimum(0.001);
