...
void OverlapQCDJetEnDown()
{
	...
	Float_t value=0;
	vector<float> xsecs_;
	ifstream ifs("QCDsamplesJetEnDown");
	string line;
	...
	string fout = "QCD_JetEnDown_B.root";
...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	path.Remove (0, 2);
				...
				TFile *foutA = TFile::Open ("QCD_JetEnDown_A.root", "RECREATE");
				TFile *foutC = TFile::Open ("QCD_JetEnDown_C.root", "RECREATE");
				TFile *foutD = TFile::Open ("QCD_JetEnDown_D.root", "RECREATE");
	...
	ofstream tfile;
	TFile * filee = new TFile("QCD_JetEnDown_B.root","update");
	filee->cd("eltau");
	//target->ls();
