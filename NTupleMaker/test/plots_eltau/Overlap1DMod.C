...
void Overlap1DMod(string syst="Nominal", string reg="Syst")
{
...
*/		
		...
		//	cout<<" Non signal filename and syst METUp or METDown "<<endl;
			...
			titles.push_back(dataname+"Nominal_B.root");
		}
...
////////////////hrest includes single-top VV
			...
			if (std::string::npos != title_.find("DataDriven"))  {col= mycolorfakes;hfakes->Add(h2,1); hfakes->SetLineColor(col); }
		...
		if (std::string::npos != n.find(variable) && obj)
		{
			...
			for (int sc = 0 ; sc<scales.size();sc++){
				...
				if (syst!="Nominal") smFilename =  region+"/Templ_"+variable+"_"+lumistring+"_mt_C1C1_"+region+"_"+syst+".root";
					else smFilename =  region+"/Templ_"+variable+"_"+lumistring+"_mt_C1C1_"+region+".root";
	...
	if(b_scale)		allbkg->Scale(scale);
			 ...
			 if (syst=="Nominall"){
				...
				else if (region =="Stau") SelEvents = true;
			...
			//		cout<<" will keep this bin "<<nb<<"  "<<all_->GetBinContent(nb)<<"  "<<keep_bin.size()<<" for Region  "<<region<<endl;
			 ...
			 if (syst!="Nominall"){
				...
				htest_rest->SetBinContent(nbb+1,0.);
				htest_fakes->SetBinContent(nbb+1,0.);
				htest_ttx->SetBinContent(nbb+1,0.);
				htest_data->SetBinContent(nbb+1,0.);

				htest_tt->SetBinError(nbb+1,0.);
				htest_wj->SetBinError(nbb+1,0.);
				htest_dyj->SetBinError(nbb+1,0.);
				htest_ztt->SetBinError(nbb+1,0.);
				htest_stop->SetBinError(nbb+1,0.);
				htest_ww->SetBinError(nbb+1,0.);
				htest_dib->SetBinError(nbb+1,0.);
				htest_rest->SetBinError(nbb+1,0.);
				htest_fakes->SetBinError(nbb+1,0.);
				htest_ttx->SetBinError(nbb+1,0.);
				htest_data->SetBinError(nbb+1,0.);
				...
				*/
				...
				//hfakes->SetBinContent(2,hfakes->GetBinContent(2)+8);
	...
	if (blind)		hh[1] = (TH1D*) allnew_->Clone("data_obs");
				...
				cout<<" comparing MC to data  , syst "<<syst<<"  MC "<<allnew_->GetSumOfWeights()<<"  data  "<<hh[1]->GetSumOfWeights()<<" tt "<<htt->GetSumOfWeights()<<"  "<<" wj "<<hwj->GetSumOfWeights()<<" dyj "<<hdyj->GetSumOfWeights()<<" ztt "<<hztt->GetSumOfWeights()<<" fakes "<<hfakes->GetSumOfWeights()<<" sT "<<hstop->GetSumOfWeights()<<" dib "<<hdib->GetSumOfWeights()<<" ttx "<<httx->GetSumOfWeights()<<" ww "<<hww->GetSumOfWeights()<<endl;
...
ModifyHist (TH1D* &h, int cl_ ,float & lumi,float & weight,string & title_, bool norm_=false)
{
	...
	int col=cl_;
			...
			if (std::string::npos != title_.find("DataDriven"))  {col= mycolorfakes; h->SetEntries(h->GetSumOfWeights());}
