...
//		if ( string::npos == nn.find("CutFlowUnW")) flcont=false;
	//	if (string::npos == nn.find("1D_") ) {MaxEventsBin = -1;drawData = false;  flcont=false;};
		...
		if (string::npos == nn.find("BDTeltau") ||  string::npos == nn.find("_16")) flcont=false;
		//if (string::npos != nn.find("Delta") ) continue;
//		if (string::npos == nn.find("_12") && string::npos == nn.find("_6")  && string::npos == nn.find("_7") && string::npos == nn.find("_8") && string::npos == nn.find("_9")) flcont=false;
...
//if (string::npos == nn.find("_12") && string::npos == nn.find("_13"))  flcont=false;
		if (!flcont) continue;
...
//cout <<xsecc <<"  "<< Lumi << "   "<<nGenn <<endl;
		...
		//	cout << h2->GetSumOfWeights()<<endl;
						...
						if (cl==2){
							...
							hqcd->Reset();
									...
									//	cout<< " for "<<fname<<"  " <<"  there xsec is "<<xsecc<<" SuOfWeights "<<nGenn<<" Lumi "<<Lumi<<" lumiweight "<<lumiweights[cl-1]<<" weight "<<float(xsecc*Lumi/nGenn)<<"  norm "<<normm<<" cl "<<cl<<"  "<<h2->GetSumOfWeights()<<endl;
						//<< " for "<<fname<<" file, there xsec is "<<xsecc<<" SuOfWeights "<<"  "<<h2->GetSumOfWeights()<<" Lumi "<<Lumi<<" lumiweight "<<lumiweights[cl-1]<<" weight "<<float(xsecc*Lumi/nGenn)<<endl;
	...
	/////////////SinglTop
		...
		if (obj->IsA ()->InheritsFrom ("TH1")) {
			...
			//pie3->Draw("3d");
			nan_.erase(0,4);
			sprintf(savename,"TauWDD/Pie_eltau.pdf",nan_);
...
//				legend_c1->AddEntry(hh[6], "C1N2(400,125)", "LEP");
			...
			if (std::string::npos != nn.find("BDT")) sprintf(nnn,"Entries");
...
{//			if (std::string::npos != nn.find("BDTeltau_")) hh[1]->GetXaxis()->SetRange(0,17);
...
if (SO_){
		...
		//	allbkg->Sumw2();
			for (int iB=1; iB<=hsum->GetNbinsX(); ++iB) {
				...
				//if (std::string::npos != nn.find("BDTeltau_") && iB>17) {datE = 0;bkgErr=0;ratio->SetBinContent(iB,0);}
...
//			if (std::string::npos != title1.find("MCTb"))  h->GetXaxis()->SetTitle("M_{CTb} [GeV]");
			...
			if (std::string::npos != title1.find("MTeltau"))  h->GetXaxis()->SetTitle("M_{T}(#mu,#tau) [GeV]");
			...
			if (std::string::npos != title1.find("BDTeltau_Stau100"))  {h->GetXaxis()->SetTitle("BDT (100,1)");}
			if (std::string::npos != title1.find("BDTeltau_Stau150"))  h->GetXaxis()->SetTitle("BDT (150,1)");
			if (std::string::npos != title1.find("BDTeltau_Stau200"))  h->GetXaxis()->SetTitle("BDT (200,1)");
			if (std::string::npos != title1.find("BDTeltau_Stau300"))  h->GetXaxis()->SetTitle("BDT (300,1)");
