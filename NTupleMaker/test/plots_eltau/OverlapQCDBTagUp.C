...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	path.Remove (0, 2);
				...
				TFile *foutA = TFile::Open ("QCD_BTagUp_A.root", "RECREATE");
				TFile *foutC = TFile::Open ("QCD_BTagUp_C.root", "RECREATE");
				TFile *foutD = TFile::Open ("QCD_BTagUp_D.root", "RECREATE");
	...
	ofstream tfile;
	TFile * filee = new TFile("QCD_BTagUp_B.root","update");
	filee->cd("eltau");
	//target->ls();
