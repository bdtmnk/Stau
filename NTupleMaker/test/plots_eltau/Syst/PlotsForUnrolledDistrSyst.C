...
void PlotsForUnrolledDistrSyst(TString directory = "",
	  ...
	  TString File = "Var_BDTeltau_Stau100_16_35invfb_mt_SR",
	  string varN  = "Var_BDTeltau_Stau100_16_35invfb_mt_SR",
	  TString variable = "",
	  TString Suffix = "Syst",
	  TString xtitle = "",
...

{
	  ...
	  vars_.push_back("BDTeltau_Stau100_16");
	//  vars_.push_back("BDTeltau_stau150_LSP1My_15");
	//  vars_.push_back("BDTeltau_stau200_LSP1My_15");
	//  vars_.push_back("BDTeltau_stau300_LSP1My_15");
...
for (int syst=0;syst<syst_.size();++syst)
   ...
   bkgdErrSyst->GetXaxis()->SetTitleOffset(1.2);
    ...
    if ( std::string::npos != variable_.find("BDT"))	 xtitle_="BDT";
    		if (std::string::npos != varN.find("BDTeltau_Stau100"))  xtitle_="BDT (100,1)";
			if (std::string::npos != varN.find("BDTeltau_Stau150")) xtitle_="BDT (150,1)";
			if (std::string::npos != varN.find("BDTeltau_Stau200")) xtitle_="BDT (200,1)";
			if (std::string::npos != varN.find("BDTeltau_Stau300_LSP1")) xtitle_="BDT (300,1)";
...
//    cout<<ratioErrHStat->GetBinError(1)<<"  "<<ratioErrHStat->GetBinError(3)<<endl;
    ...
    if (variable_== "MET" || variable_== "Mt2lestermuel" || variable_== "Mt2lestereltau" || variable_== "Mt2lestereltau"){  pads[1]->SetLogx();pads[0]->SetLogx();}
    string v="";
