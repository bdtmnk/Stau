...
void PlotsForUnrolledDistr(TString directory = "",
	  TString suffix = "_SR_CR1_MuTau_all_wBinUncrt",
...
//		BinLabels[i]=name;
    ...
    TH1D *TT,*WJ,*DY,*ZTT,*ST,*VV,*WW,*TTX,*Fakes;
    int nB=1;
    ...
    fileStat = new TFile("/nfs/dust/cms/user/alkaloge/Workdir/CMSSW_7_4_7/src/Limits/STau/Results_SR_CR1/mlfit_met_MT2lester_DZeta01J1D_17_mt_only.root","read");
...
for (int svar=0;svar<vars_.size();++svar)
{
    ...
    TH1D * WJq = (TH1D*)filee->Get("shapes_prefit/ch1/tt_"+Variable); ///dummy
    ...
    TH1D * VVq = (TH1D*)filee->Get("shapes_prefit/ch1/rest_"+Variable);
    TH1D * WWq = (TH1D*)filee->Get("shapes_prefit/ch1/ww_"+Variable);
    TH1D * TTXq = (TH1D*)filee->Get("shapes_prefit/ch1/ttx_"+Variable);
    TH1D * Fakesq = (TH1D*)filee->Get("shapes_prefit/ch1/fakes_"+Variable);
    TH1D * histSignal = (TH1D*)file->Get("C1N2_100_LSP1_B_"+Variable);
    TH1D * histSignal2 = (TH1D*)file->Get("C1N2_100_LSP1_B_"+Variable);
    TH1D * histSignal3 = (TH1D*)file->Get("C1N2_100_LSP1_B_"+Variable);
//    TH1D * histSignal2 = (TH1D*)file->Get("C1N2_300_LSP200_B_"+Variable);
//    TH1D * histSignal3 = (TH1D*)file->Get("C1N2_500_LSP200_B_"+Variable);
	WJq->Reset();
    ...
    for (int nb=1;nb <= fitHisto->GetNbinsX()+1; ++nb){ 
	   ...
	   Fakes ->SetBinContent(nb,Fakesq->GetBinContent(nb)); Fakes->SetBinError(nb,Fakesq->GetBinError(nb)); 
...
/*
    ...
    TH1D * Fakes = (TH1D*)file->Get("fakes_"+Variable);
...
//   allbkg ->SetName("allbkg");
 ...
 if (!OverlayFit) { fitHistoPostH0 = (TH1D*)allbkg->Clone();  fitHistoPostH1 = (TH1D*)allbkg->Clone();}
  ...
  std::cout << "Fakes   : " << Fakes->GetSumOfWeights() << " : " <<" GetNbinsX= " << Fakes->GetNbinsX()<< "  Purity  "<<Fakes->GetSumOfWeights()/allbkg->GetSumOfWeights()<< std::endl;
...
cout<<" check "<<endl;
    ...
    legend->AddEntry(Fakes,"Jet#rightarrow#tau-Fakes","f");
    ...
    legend->AddEntry(histSignal2,"C1N2_300_LSP_200","ple");
...
//for (int iB=1; iB<=nBins; ++iB) {WJ->SetBinError(iB,0);TT->SetBinError(iB,0);DY->SetBinError(iB,0);Fakes->SetBinError(iB,0);VV->SetBinError(iB,0);TTX->SetBinError(iB,0);}
    WJ->SetMarkerStyle(0);
    TT->SetMarkerStyle(0);
    TTcl->SetMarkerStyle(0);
    DY->SetMarkerStyle(0);
    ZTT->SetMarkerStyle(0);
    Fakes->SetMarkerStyle(0);
    VV->SetMarkerStyle(0);
    WW->SetMarkerStyle(0);
...
cout<<"	 SR bin    &  TTJets     & WJets    & DYJets       & Fakes   & SingleT    & VVJets   & TTXJets       & Total Bkg ($\\pm total error)    & Observed \\\\"<<endl;
     cout<<" \\hline "<<endl;

     for (int b=1;b<allbkg->GetNbinsX()+1;++b){
float sumBin = TTcl->GetBinContent(b)+WJ->GetBinContent(b)+DY->GetBinContent(b)+ZTT->GetBinContent(b)+Fakes->GetBinContent(b)+ST->GetBinContent(b)+VV->GetBinContent(b)+WW->GetBinContent(b)+TTX->GetBinContent(b);
...
//    float shapes_err =sqrt(total_err*total_err - norm_err*norm_err);
     ...
     //cout<<"SL"<<histData->GetXaxis()->GetBinLabel(b)<<" & "<<TTcl->GetBinContent(b)<< " & "<<WJ->GetBinContent(b)<<" & "<<DY->GetBinContent(b)+ZTT->GetBinContent(b)<<" & "<<Fakes->GetBinContent(b)<<" & "<<ST->GetBinContent(b)<<" & "<<VV->GetBinContent(b)+WW->GetBinContent(b)<<" & "<<TTX->GetBinContent(b)<<" & "<<sumBin<<" $\\pm$ "<<sqrt(sumBin)<<" $\\pm$ "<< errNormOnlyB->GetBinError(b)<<" & "<<histData->GetBinContent(b)<<" \\\\\\hline "<<endl;
     //cout<<histData->GetXaxis()->GetBinLabel(b)<<" & "<<TTcl->GetBinContent(b)<< " & "<<WJ->GetBinContent(b)<<" & "<<DY->GetBinContent(b)+ZTT->GetBinContent(b)<<" & "<<Fakes->GetBinContent(b)<<" & "<<ST->GetBinContent(b)<<" & "<<VV->GetBinContent(b)+WW->GetBinContent(b)<<" & "<<TTX->GetBinContent(b)<<" & "<<sumBin<<" $\\pm$ "<<norm_err<<" $\\pm$ "<< shapes_err<<" & "<<histData->GetBinContent(b)<<" \\\\\\hline "<<endl;
 //   std::cout<<histData->GetXaxis()->GetBinLabel(b)<<" & "<<std::fixed<<std::setprecision(1)<<TTcl->GetBinContent(b)<< " & "<<std::setprecision(1)<<WJ->GetBinContent(b)<<" & "<<std::setprecision(1)<<DY->GetBinContent(b)+ZTT->GetBinContent(b)<<" & "<<std::setprecision(1)<<Fakes->GetBinContent(b)<<" & "<<std::setprecision(1)<<ST->GetBinContent(b)<<" & "<<std::setprecision(1)<<VV->GetBinContent(b)+WW->GetBinContent(b)<<" & "<<std::setprecision(1)<<TTX->GetBinContent(b)<<" & "<<std::setprecision(1)<<sumBin<<" $\\pm$ "<<std::setprecision(1)<<total_err<<" & "<< std::setprecision(0)<<histData->GetBinContent(b)<<" \\\\\\hline "<<std::endl;
	    ...
	    << " & "<<std::setprecision(1)<<Fakes->GetBinContent(b)<<" $\\pm$ "<<Fakes->GetBinError(b)
