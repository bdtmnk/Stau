void CreateDatacards_VARIABLE_CHANNEL_LUMI() {
...
bool bFirst = FLAGG;
  ...
  //TH1D * rest_bkg = (TH1D*)file->Get("1D_rest_bkg_VARIABLE");
  TH1D * rest_bkg = (TH1D*)file->Get("1D_dib_VARIABLE");
  TH1D * ttx = (TH1D*)file->Get("1D_ttx_VARIABLE");
  TH1D * stop = (TH1D*)file->Get("1D_sT_VARIABLE");
  rest_bkg ->Add(ttx,1);
  rest_bkg ->Add(stop,1);
  rest_bkg->SetName("1D_rest_bkg_VARIABLE");
  TH1D * tt = (TH1D*)file->Get("1D_tt_VARIABLE");
  TH1D * wj = (TH1D*)file->Get("1D_wj_VARIABLE");
  TH1D * dyj = (TH1D*)file->Get("1D_dyj_VARIABLE");
  TH1D * qcd = (TH1D*)file->Get("1D_qcd_VARIABLE");
...
/*
  for (int iB=1; iB<=nBins; ++iB) {
    TH1D * ttUp = (TH1D*)tt->Clone("1D_tt_VARIABLE_ttBin"+binName[iB]+"Up");
    TH1D * ttDown = (TH1D*)tt->Clone("1D_tt_VARIABLE_ttBin"+binName[iB]+"Down");
    float xtt = ttUp->GetBinContent(iB);
    float ett = ttUp->GetBinError(iB);
    ttUp->SetBinContent(iB,xtt+ett);
    ttDown->SetBinContent(iB,xtt-ett);
    ttUp->Write("1D_tt_VARIABLE_ttBin"+binName[iB]+"Up");
    ttDown->Write("1D_tt_VARIABLE_ttBin"+binName[iB]+"Down");
	

    TH1D * WUp = (TH1D*)wj->Clone("1D_wj_VARIABLE_wjBin"+binName[iB]+"Up");
    TH1D * WDown = (TH1D*)wj->Clone("1D_wj_VARIABLE_wjBin"+binName[iB]+"Down");
    float xwj = WUp->GetBinContent(iB);
    float ewj = WUp->GetBinError(iB);
    WUp->SetBinContent(iB,xwj+ewj);
    WDown->SetBinContent(iB,xwj-ewj);
    WUp->Write("1D_wj_VARIABLE_wjBin"+binName[iB]+"Up");
    WDown->Write("1D_wj_VARIABLE_wjBin"+binName[iB]+"Down");

    TH1D * dyjUp = (TH1D*)dyj->Clone("1D_dyj_VARIABLE_dyjBin"+binName[iB]+"Up");
    TH1D * dyjDown = (TH1D*)dyj->Clone("1D_dyj_VARIABLE_dyjBin"+binName[iB]+"Down");
    ...
    dyjUp->Write("1D_dyj_VARIABLE_dyjBin"+binName[iB]+"Up");
    dyjDown->Write("1D_dyj_VARIABLE_dyjBin"+binName[iB]+"Down");


    TH1D * rest_bkgUp = (TH1D*)rest_bkg->Clone("1D_rest_bkg_VARIABLE_rest_bkgBin"+binName[iB]+"Up");
    TH1D * rest_bkgDown = (TH1D*)rest_bkg->Clone("1D_rest_bkg_VARIABLE_rest_bkgBin"+binName[iB]+"Down");
    ...
    rest_bkgUp->Write("1D_rest_bkg_VARIABLE_rest_bkgBin"+binName[iB]+"Up");
    rest_bkgDown->Write("1D_rest_bkg_VARIABLE_rest_bkgBin"+binName[iB]+"Down");

    TH1D * qcdUp = (TH1D*)qcd->Clone("1D_qcd_VARIABLE_qcdBin"+binName[iB]+"Up");
    TH1D * qcdDown = (TH1D*)qcd->Clone("1D_qcd_VARIABLE_qcdBin"+binName[iB]+"Down");
    ...
    qcdUp->Write("1D_qcd_VARIABLE_qcdBin"+binName[iB]+"Up");
    qcdDown->Write("1D_qcd_VARIABLE_qcdBin"+binName[iB]+"Down");
...
*/
 ...
 TString sys = Systematics[j].c_str();
  ...
  TFile *filesyst = TFile::Open ("FILE_"+sys+".root", "read");

if (bFirst){

  TH1D * tt_syst = (TH1D*)filesyst->Get("1D_tt_VARIABLE_"+sys);
  TH1D * wj_syst = (TH1D*)filesyst->Get("1D_wj_VARIABLE_"+sys);
  TH1D * dyj_syst = (TH1D*)filesyst->Get("1D_dyj_VARIABLE_"+sys);
  TH1D * qcd_syst = (TH1D*)filesyst->Get("1D_qcd_VARIABLE_"+sys);
  TH1D * dib_syst = (TH1D*)filesyst->Get("1D_dib_VARIABLE_"+sys);
  TH1D * sT_syst = (TH1D*)filesyst->Get("1D_sT_VARIABLE_"+sys);
  //TH1D * rest_bkg_syst = (TH1D*)filesyst->Get("1D_rest_bkg_VARIABLE_"+sys);
  TH1D * rest_bkg_syst = (TH1D*)filesyst->Get("1D_ttx_VARIABLE_"+sys);
  rest_bkg_syst->Add( dib_syst,1);
  rest_bkg_syst->Add( sT_syst,1);
  rest_bkg_syst->SetName("1D_rest_bkg_VARIABLE_"+sys);
  ...
  TH1D * signal_syst = (TH1D*)filesyst->Get("1D_SIGNAL_B_VARIABLE_"+sys);
  fileout->cd();
...
/*
  ...
  for (int iB=1; iB<=nBins; ++iB) {
    TH1D * signalUp = (TH1D*)signal->Clone("1D_SIGNAL_B_VARIABLE_signalBin"+binName[iB]+"Up");
    TH1D * signalDown = (TH1D*)signal->Clone("1D_1D_SIGNAL_B_VARIABLE_signalBin"+binName[iB]+"Down");
...
//if (bFirst){
    signalUp->Write("1D_SIGNAL_B_VARIABLE_signalBin"+binName[iB]+"Up");
    signalDown->Write("1D_SIGNAL_B_VARIABLE_signalBin"+binName[iB]+"Down");
//	}
  }
...
//  textFile << "observation " << data_obs->Integral() << std::endl;
  textFile << "-----------------" << std::endl;
  textFile << "shapes * *  FILEOUT.root  $PROCESS    $PROCESS_$SYSTEMATIC " << std::endl;
  textFile << "-----------------" << std::endl;
  textFile << "bin                  CHANNEL	CHANNEL	CHANNEL	CHANNEL	CHANNEL CHANNEL" << std::endl;
  textFile << "process              1D_SIGNAL_B_VARIABLE 1D_tt_VARIABLE 1D_wj_VARIABLE 1D_dyj_VARIABLE 1D_rest_bkg_VARIABLE 1D_qcd_VARIABLE" << std::endl;
  ...
  textFile << "dy_norm         lnN   	-       -	-	1.03	-	-" << std::endl;
  ...
  textFile << "TopPt         	shape  	1	1	1	1	1    	1"<< std::endl;
  textFile << "ZPt         	shape  	1	1	1	1	1    	1"<< std::endl;
  textFile << std::endl;
 /* for (int iB=1; iB<=nBins; ++iB) {
