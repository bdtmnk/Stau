...
void OverlapQCDZPtDown()
{
	...
	Float_t value=0;
	vector<float> xsecs_;
	ifstream ifs("QCDsamplesZPtDown");
	string line;
	...
	string fout = "QCD_ZPtDown_B.root";
...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	path.Remove (0, 2);
				...
				TFile *foutA = TFile::Open ("QCD_ZPtDown_A.root", "RECREATE");
				TFile *foutC = TFile::Open ("QCD_ZPtDown_C.root", "RECREATE");
				TFile *foutD = TFile::Open ("QCD_ZPtDown_D.root", "RECREATE");
	...
	ofstream tfile;
	TFile * filee = new TFile("QCD_ZPtDown_B.root","update");
	filee->cd("mutau");
	//target->ls();
