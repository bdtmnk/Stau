	...
	while ((key = (TKey *) nextkey ())) {
		...
		if (obj) {
			...
			if (cl=1){
				...
				legend_c1->AddEntry(hh[3], "#tilde{#tau}(110,1) X10", "LEP");
				legend_c1->AddEntry(hh[4], "#tilde{#tau}(120,1) X10", "LEP");
				legend_c1->AddEntry(hh[5], "#tilde{#tau}(150,1) X10", "LEP");
				legend_c1->AddEntry(hh[6], "#tilde{#tau}(200,1) X10", "LEP");
				legend_c1->AddEntry(hh[7], "#tilde{#tau}(250,1) X10", "LEP");
				legend_c1->AddEntry(hh[8], "#tilde{#tau}(300,1) X10", "LEP");
				legend_c1->AddEntry(hh[9], "#tilde{#tau}(350,1) X10", "LEP");
				legend_c1->AddEntry(hh[10], "#tilde{#tau}(400,1) X10", "LEP");
				legend_c1->AddEntry(hh[11], "#tilde{#tau}(450,1) X10", "LEP");
				legend_c1->AddEntry(hh[12], "#tilde{#tau}(500,1) X10", "LEP");
			...
			sig->GetXaxis()->SetRange(hh[1]->FindFirstBinAbove(0),hh[1]->FindLastBinAbove(5));
			...
			*/
			...
			if (!norm_)sprintf(f,"TauZeroLSP/%sProbe.pdf",namee);
			else sprintf(f,"TauZeroLSP/%s_Norm.pdf",namee);

			if (!norm_)sprintf(ff,"TauZeroLSP2/%s_Log.pdf",namee);
			else sprintf(ff,"TauZeroLSP2/%s_Log_Norm.pdf",namee);
