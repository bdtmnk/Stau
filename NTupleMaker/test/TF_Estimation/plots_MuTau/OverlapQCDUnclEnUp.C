...
void OverlapQCDUnclEnUp()
{
	...
	Float_t value=0;
	vector<float> xsecs_;
	ifstream ifs("QCDsamplesUnclEnUp");
	string line;
	...
	string fout = "QCD_UnclEnUp_B.root";
...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	path.Remove (0, 2);
				...
				TFile *foutA = TFile::Open ("QCD_UnclEnUp_A.root", "RECREATE");
				TFile *foutC = TFile::Open ("QCD_UnclEnUp_C.root", "RECREATE");
				TFile *foutD = TFile::Open ("QCD_UnclEnUp_D.root", "RECREATE");
	...
	ofstream tfile;
	TFile * filee = new TFile("QCD_UnclEnUp_B.root","update");
	filee->cd("mutau");
	//target->ls();
