...
void OverlapQCDMETRecoilUp()
{
	...
	Float_t value=0;
	vector<float> xsecs_;
	ifstream ifs("QCDsamplesMETRecoilUp");
	string line;
	...
	string fout = "QCD_METRecoilUp_B.root";
...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	path.Remove (0, 2);
				...
				TFile *foutA = TFile::Open ("QCD_METRecoilUp_A.root", "RECREATE");
				TFile *foutC = TFile::Open ("QCD_METRecoilUp_C.root", "RECREATE");
				TFile *foutD = TFile::Open ("QCD_METRecoilUp_D.root", "RECREATE");
	...
	ofstream tfile;
	TFile * filee = new TFile("QCD_METRecoilUp_B.root","update");
	filee->cd("mutau");
	//target->ls();
