...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	while ((key = (TKey *) nextkey ())) {
		...
		string nn = obj->GetName();
			...
			if ( string::npos == nn.find("met_")   && string::npos == nn.find("DZeta_")  && string::npos == nn.find("MTsum_M") && string::npos == nn.find("MTtot_M") && string::npos == nn.find("MT2lester_") ) continue;
...
//		if (obj->IsA ()->InheritsFrom ("TH1")  || obj->IsA ()->InheritsFrom ("TTree")) cout<<" TH1 is not valid "<<endl;
		...
		//if (obj->IsA ()->InheritsFrom ("TH1") && string::npos == nn.find("histWeights") )  continue;
	//	if (obj->IsA ()->InheritsFrom ("TH1D") ) continue;
		if (obj->IsA ()->InheritsFrom ("TH2D") )  continue;
		...
		if (obj->IsA ()->InheritsFrom ("TH3") ) {
			...
			while (nextsource) {
				...
				if (key2) {
					...
					if (cl==2){	
						allRegC  = (TH3D*) h2->Clone();
					}
					if (cl==3){	
						allRegD  = (TH3D*) h2->Clone();
					}
					if (cl==4){	
