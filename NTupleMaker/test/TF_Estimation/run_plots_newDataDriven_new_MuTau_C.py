...
os.system("cd /nfs/dust/cms/user/dydukhle/STAU/Analysis18/CMSSW_10_2_16/src/DesyTauAnalyses/NTupleMaker/test/TF_Estimation/")
#/nfs/dust/cms/user/dydukhle/CMSSW_9_4_0_patch1/src/DesyTauAnalyses/NTupleMaker/test")#;eval `scramv1 runtime -sh` ;
dir="/nfs/dust/cms/user/dydukhle/STAU/Analysis18/CMSSW_10_2_16/src/DesyTauAnalyses/NTupleMaker/test/TF_Estimation/"
i=0
print(arguments)
...
for i in range(len(content)):
    ...
    path = "dir_{0}_MuTau_{2}".format(fileB, channel, syst)
    try:
        os.mkdir(path)
    ...
    print("echo the signal filein : {0} , the fileout : {1}_C.root normal Tau".format(file, fileB))
    if os.path.exists("./plots_MuTau/{0}_C.root".format(fileB)):continue
    os.system("mkdir {0}".format(working_dir))
    os.system("cp {0}/analyzer_MuTau_h {0}/{1}/analyzer.h".format(dir, working_dir))
    os.system("cp {0}/analyzer_MuTau_TF_C {0}/{2}/analyzer.C".format(dir, channel, working_dir))
    ...
    try:os.system("cd {0}/{1}".format(dir, working_dir))
    except Exception as e: print("Exception {0}".format(e))
    else:os.chdir("{0}/{1}".format(dir, working_dir))
    ...
    os.system("sed -i 's/SIGNHERE/{0}/g' analyzer.C".format('SS', working_dir))
    os.system("sed -i 's/CHANNELHERE/{0}/g' analyzer*.*".format(channel, working_dir))
    os.system("sed -i 's/DIRHERE/{0}/g' analyzer*.*".format(dirhere, working_dir))
    ...
    os.system("mv {0}/{3}/plots.root {0}/plots_MuTau/{2}_C.root".format(dir, channel, fileB, working_dir))
