...
/*
	...
	{
  if (   std::string::npos != titles[k].find(sign_) || std::string::npos != titles[k].find(signn_)){
	n_ = titles[k];
...
*/
    ...
    lumiweights.push_back(float(norm));
              ...
              cout<< " for first source file, there where "<<nGen<<" events with xsec "<<xsec<<" weight "<<lumiweights[0]<<"  "<<Lumi<<"  "<<nGen<<endl;//" weight "<<float(xsec*Lumi/nGen)<<"  norm "<<norm<<endl;
       ...
       // first_source->cd (path);
        ...
        TObject *obj = key->ReadObj ();
	     string nn = obj->GetName();
            if (std::string::npos != nn.find("hDZeta_19") ) break;
