...
void OverlapSyst()
{
	...
	Float_t value=0;
	vector<float> xsecs_;
	ifstream ifs("datasetsSyst");
	if( QCDMC)  ifstream ifs("datasetsQCD");
	string line;

	string syst = "UnclEnUp";
	while(std::getline(ifs, line)) // read one line from ifs
	{
		...
		titles.push_back(dataname+"_B_OS_Nominal_B.root");
	...
	ifstream ifs2("datasetsSyst");
	string line2;

	while(std::getline(ifs2, line2)) // read one line from ifs
	{
		istringstream iss(line2); // access line as a stream
		string dataname;
		float XSec;	
		float xs,fact,fact2,fact3;
		xs=0;fact=1;fact2=1;fact3=1;
		iss >> dataname >> xs >> fact >> fact2 >> fact3;
		//if (std::string::npos != dataname.find("Single")) continue; 
		titles.push_back(dataname+"_B_OS_Nominal_A.root");
		XSec= xs*fact*fact2*fact3;
		cout<<" Found the correct cross section "<<xs<<" for Dataset "<<dataname<<" XSec "<<XSec<<"  "<<fact<<"  "<<fact2<<"  "<<fact<<endl;
		xsecs_.push_back(XSec);
	}
	...
	void
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs, string syst)
{
	cout << "	" << "========================================================" << endl;
...
//cout << normm2<<"    !!!!!!!!!!!!! "<< endl;
	...
	TH1D* allbkg, *htt,*hstop,*hwj,*hdyj,*hztt,*hrare,*hdib,*hww,*hqcd,*httx,*hMC, *hSysUp, *hSysDown;
	...
	while ((key = (TKey *) nextkey ())) {
		...
		//if (string::npos == nn.find("TaupT_15")) flcont=false;
		//if (string::npos == nn.find("_17") && string::npos == nn.find("_15")) flcont=false;
...
if (!SO_) legend_c1    = new TLegend (0.55, 0.92, 0.9, 0.55);
...
if (SO_)
			...
			for (int ib=20;ib<hh[1]->GetNbinsX();ib++) 
			...
			string fname= nextsource->GetName();
				  ...
				  if  ( std::string::npos == fname.find("TT_TuneCUETP8M2T4_13TeV-powheg-pythia8") ) eventCountt = (TH1D*)nextsource->Get(Channel+"/histWeightsH");
				  if ( std::string::npos != fname.find("TT_TuneCUETP8M2T4_13TeV-powheg-pythia8") ) eventCountt = (TH1D*)nextsource->Get(Channel+"/histTopPt");
					...
					if (key2) {
						...
						if (cl==2){
							...
							hqcd  = (TH1D*) h2->Clone();
							hMC  = (TH1D*) h2->Clone();
							hSysUp  = (TH1D*) h2->Clone();
							hSysDown  = (TH1D*) h2->Clone();
						...
						//cout<< " for "<<fname<<" file, there xsec is "<<xsecc<<" SuOfWeights "<<"  "<<h2->GetSumOfWeights()<<" Lumi "<<Lumi<<" lumiweight "<<lumiweights[cl-1]<<" weight "<<float(xsecc*Lumi/nGenn)<< " nGenn " << nGenn <<endl;
    ...
    h2->Draw();
    canv1->cd();
	TString title2 = title_;
    canv1->Print(title2 +".pdf");
*/

if (std::string::npos != title_.find(syst))
{
...
{
	if (std::string::npos == title_.find("Singl")){hMC->Add(h2); cout<<"DaDADA!!!!!   "<< fname<<endl;

			if (std::string::npos != title_.find("wJets")|| std::string::npos != title_.find("WJetsToLNu") || std::string::npos != title_.find("W1JetsToLNu") || std::string::npos != title_.find("W2JetsToLNu") 
			|| std::string::npos != title_.find("W3JetsToLNu") || std::string::npos != title_.find("W4JetsToLNu"))  { 
			cout << h2->GetSumOfWeights()<<endl;
			h2->Scale(1.1); hSysUp->Add(h2); h2->Scale(1/1.1); 
			h2->Scale(0.9); hSysDown->Add(h2); h2->Scale(1/0.9);
			cout << h2->GetSumOfWeights()<<endl;}
			if (std::string::npos != title_.find("TT_TuneCUETP8M2T4_13TeV-powheg-pythia8") || std::string::npos != title_.find("TTPow")) {
			h2->Scale(1.05); hSysUp->Add(h2); h2->Scale(1/1.05); 
			h2->Scale(0.95); hSysDown->Add(h2); h2->Scale(1/0.95);}
			if (std::string::npos != title_.find("QCD"))  {
			h2->Scale(1.5); hSysUp->Add(h2); h2->Scale(1/1.5); 
			h2->Scale(0.5); hSysDown->Add(h2); h2->Scale(1/0.5);}

			if ( (std::string::npos != title_.find("DYJets") || std::string::npos != title_.find("DY1Jets") || 
			std::string::npos != title_.find("DY2Jets") || std::string::npos != title_.find("DY3Jets") || std::string::npos != title_.find("DY4Jets") )&& std::string::npos == title_.find("isZTT") )  {
			h2->Scale(1.02); hSysUp->Add(h2); h2->Scale(1/1.02); 
			h2->Scale(0.98); hSysDown->Add(h2); h2->Scale(1/0.98);}
			if (std::string::npos != title_.find("isZTT") ){
			h2->Scale(1.02); hSysUp->Add(h2); h2->Scale(1/1.02); 
			h2->Scale(0.92); hSysDown->Add(h2); h2->Scale(1/0.92);} 
	
		if (std::string::npos != title_.find("ST_") || std::string::npos != title_.find("channel") )  {
			h2->Scale(1.5); hSysUp->Add(h2); h2->Scale(1/1.5); 
			h2->Scale(0.5); hSysDown->Add(h2); h2->Scale(1/0.5);}
			if ( (std::string::npos != title_.find("WW") || std::string::npos != title_.find("ZZ") ||  std::string::npos != title_.find("WZ") || std::string::npos != title_.find("WG") || std::string::npos != title_.find("ZG") ) && std::string::npos == title_.find("WWTo") ) {
			h2->Scale(1.5); hSysUp->Add(h2); h2->Scale(1/1.5); 
			h2->Scale(0.5); hSysDown->Add(h2); h2->Scale(1/0.5);}
			if ( std::string::npos != title_.find("WWTo")  ) {
			h2->Scale(1.5); hSysUp->Add(h2); h2->Scale(1/1.5); 
			h2->Scale(0.5); hSysDown->Add(h2); h2->Scale(1/0.5);}

			if ( std::string::npos != title_.find("TTW") || std::string::npos != title_.find("TTZ") || std::string::npos != title_.find("tZq") || std::string::npos != title_.find("TG") || std::string::npos != title_.find("tG") || std::string::npos != title_.find("TTG") || std::string::npos != title_.find("ttW") || std::string::npos != title_.find("ttZ") || std::string::npos != title_.find("tZ") || std::string::npos != title_.find("TTT_") ) {
			h2->Scale(1.5); hSysUp->Add(h2); h2->Scale(1/1.5); 
			h2->Scale(0.5); hSysDown->Add(h2); h2->Scale(1/0.5);}
...
/*
			///////// TPie
			Float_t vals[]={hdib->GetSumOfWeights()+httx->GetSumOfWeights(), hwj->GetSumOfWeights(), hww->GetSumOfWeights(), htt->GetSumOfWeights()+hstop->GetSumOfWeights(),  hdyj->GetSumOfWeights()+hztt->GetSumOfWeights(),hqcd->GetSumOfWeights()};
			...
			Int_t colors[] = { hdib->GetLineColor(),hwj->GetLineColor(), hww->GetLineColor(),htt->GetLineColor(),hdyj->GetLineColor(),hqcd->GetLineColor()};
			...
			//pie3->Draw("3d");
			nan_.erase(0,4);
			sprintf(savename,"Tau/Pie%s.pdf",nan_);
...
*/
	...
	if (drawData)	pad2->Draw();
			...
			OverFlow(hMC,hh[1]->FindLastBinAbove(MaxEventsBin));
...
//if (drawData)			legend_c1->AddEntry(hMC, "Nominal MC", "LEP");
				char lab_[100];
/*if (SO_){
				legend_c1->AddEntry(hh[2], "#tilde{#tau}_{L}(100,1)", "LEP");
				legend_c1->AddEntry(hh[3], "C1N2(100,1)", "LEP");
				legend_c1->AddEntry(hh[4], "C1C1(100,1)", "LEP");
//				legend_c1->AddEntry(hh[5], "C1N2(300,100)", "LEP");
//				legend_c1->AddEntry(hh[6], "C1N2(400,125)", "LEP");
}*/
				TString syst2 = syst;
				v1->SetLineColor(mycolortt);v1->SetFillColor(mycolortt);legend_c1->AddEntry(v1, "TTJets "+syst2, "f");
				v3->SetLineColor(mycolorst);v3->SetFillColor(mycolorst);legend_c1->AddEntry(v3, "singleTop "+syst2, "f");
				v2->SetLineColor(mycolorttx);v2->SetFillColor(mycolorttx);legend_c1->AddEntry(v2, "TTX/TG "+syst2, "f");
				v4->SetLineColor(mycolorwjet);v4->SetFillColor(mycolorwjet);legend_c1->AddEntry(v4, "WJets "+syst2, "f");
				v7->SetLineColor(mycolorztt);v7->SetFillColor(mycolorztt);legend_c1->AddEntry(v7, "DY#rightarrow#tau#tau "+syst2, "f");
				v5->SetLineColor(mycolorqcd);v5->SetFillColor(mycolorqcd);legend_c1->AddEntry(v5, "QCD "+syst2, "f");
				v6->SetLineColor(mycolordyj);v6->SetFillColor(mycolordyj);legend_c1->AddEntry(v6, "DY#rightarrow#mu#mu/ee "+syst2, "f");
				v8->SetLineColor(mycolorvv);v8->SetFillColor(mycolorvv);legend_c1->AddEntry(v8, "Rest "+syst2, "f");
				v9->SetLineColor(mycolorww);v9->SetFillColor(mycolorww);legend_c1->AddEntry(v9, "WW "+syst2, "f");
			...
			if ( std::string::npos != titlee.find("phi") || std::string::npos != titlee.find("Phi") ||  std::string::npos != titlee.find("eta") ||  std::string::npos != titlee.find("nJ") || std::string::npos != titlee.find("nB") ||  std::string::npos != titlee.find("nEl") || std::string::npos != titlee.find("nM") || std::string::npos != titlee.find("nT")) hsum->GetYaxis()->SetTitle("Entries");
...
cout<<			hMC->GetEntries()<<"  "<<hMC->GetSumOfWeights()<<endl;
			if (!norm_) hs->Draw("same hist");

			hMC->SetMarkerStyle(20);
			hMC->SetMarkerColor(kBlue);
			hMC->SetLineColor(kBlue);
			hMC->SetFillColor(kBlue);
 			//hMC->Scale(1/hMC->GetSumOfWeights());
			hMC->Draw("same ep hist");
if (drawData)			legend_c1->AddEntry(hMC, "Nominal MC", "LEP");

			if (norm_) hh[1]->Scale(1/hh[1]->Integral());
/*if (SO_){
			for (int ij=2;ij<5;ij++){
      		        	if ( string::npos == nn.find("CutFlowUnW") && string::npos == nn.find("1D_")) 
			...
			ratio->GetXaxis()->SetTitle(hMC->GetXaxis()->GetTitle());
			...
			TString syst2 =syst;
			ratio->GetYaxis()->SetTitle("MC / MC_"+syst2);
			...
			ratio->SetTitle("");
			TAxis *axis = hMC->GetXaxis();
			float halfbin = axis->GetBinWidth(hMC->GetBin(1))*0.5;
			...
			DataHErrP  = (TH1D*) hMC->Clone();
			DataHErrM  = (TH1D*) hMC->Clone();
...
cout << "SYS!!!   " << hSysUp->GetSumOfWeights()<<"   "<<hSysDown->GetSumOfWeights()<<endl;
			//hSysUp->Scale(1/hSysUp->GetSumOfWeights());
 			//hSysDown->Scale(1/hSysDown->GetSumOfWeights());

			for (int iB=1; iB<=hsum->GetNbinsX(); ++iB) {
				float datX = hMC->GetBinContent(iB);
				float datE = hMC->GetBinError(iB);
				...
				float SystUp = TMath::Abs(hMC->GetBinContent(iB)- hSysUp->GetBinContent(iB));
				float SystDown = TMath::Abs(hMC->GetBinContent(iB)- hSysDown->GetBinContent(iB));
				float Syst =  TMath::Max(SystUp,SystDown)/bkgX;
				float SystLumi = 0.026;
				float SystMuId = 0.02;
				float SystJetTau = 0.1;
				float SystLepTau = 0.1;
				float SystTauId = 0.05;
				float SystBtag = 0.04;
//cout<< iB<<endl;
//cout <<float(bkgE / bkgX) <<endl;
//cout <<Syst <<endl;
				float bkgErr = TMath::Sqrt(float(bkgE / bkgX)*float(bkgE / bkgX)+SystLumi*SystLumi+Syst*Syst+SystMuId*SystMuId+SystJetTau*SystJetTau+SystLepTau*SystLepTau+SystTauId*SystTauId+SystBtag*SystBtag);
...
*/
			...
			//namee = namee+"_"+syst;
			if (SO_ && !QCDMC)sprintf(f,"Tau/%sSO_UnclEnUp.pdf",namee);
			if (SO_ && QCDMC)sprintf(f,"Tau/%s_QCDMC_SO.pdf",namee);
			if(!SO_ && !QCDMC) sprintf(f,"Tau/%s_UnclEnUp.pdf",namee);
			if(!SO_ && QCDMC) sprintf(f,"Tau/%s_QCDMC.pdf",namee);



			//cout <<"final  "<< hsum->GetSumOfWeights()<< endl;
			//if (hsum->GetSumOfWeights()>0.99 && hsum->GetSumOfWeights()<1.01) {c1->Print (f,"pdf");}
			c1->Print (f,"pdf");
			//if (string::npos != nn.find("hDZeta_")) normm2=normm2/hsum->GetSumOfWeights();
		}
...
void ModifyHist (TH1D* &h, int cl_ ,float & lumi,float & weight,string & title_, bool norm_=false,float XSec=1.)
		{
			...
			int col=;//kOrange;
			 ...
			 if (std::string::npos != title_.find("MT2") ||  std::string::npos != title_.find("DZeta")) h->Rebin(2);
...
//			if (std::string::npos != title1.find("MCTb"))  h->GetXaxis()->SetTitle("M_{CTb} [GeV]");
			...
			if (std::string::npos != title1.find("dPhiJ0MET"))  h->GetXaxis()->SetTitle("#Delta#Phi(J_{0},#slash{E}_{T}) [rad]");
			if (std::string::npos != title1.find("dPhiJMET"))  h->GetXaxis()->SetTitle("#Delta#Phi(J,#slash{E}_{T}) [rad]");
