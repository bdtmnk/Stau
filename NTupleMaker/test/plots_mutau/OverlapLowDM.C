...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles)
{
	...
	while ((key = (TKey *) nextkey ())) {
		...
		if (string::npos == nn.find("_19") && flcont) flcont=false;
		if (!flcont) continue;
		...
		if (obj) {
			...
			*/
			if (cl=1){
				...
				//v9->SetLineColor(47);
				char lab_[100];
				//legend_c1->AddEntry(hh[2], "#(50,0)", "LEP");
				legend_c1->AddEntry(hh[3], "#(150,100)", "LEP");
				legend_c1->AddEntry(hh[4], "#(200,150)", "LEP");
				legend_c1->AddEntry(hh[5], "#(300,250)", "LEP");
				legend_c1->AddEntry(hh[6], "#(400,350)", "LEP");
				legend_c1->AddEntry(hh[7], "#(500,450)", "LEP");
/*
...
*/
			...
			*/
			...
			if (!norm_)sprintf(f,"TauLowDM/%s.pdf",namee);
			else sprintf(f,"TauLowDM/%s_Norm.pdf",namee);

			if (!norm_)sprintf(ff,"TauLowDM/%s_Log.pdf",namee);
			else sprintf(ff,"TauLowDM/%s_Log_Norm.pdf",namee);
