...
//		if (string::npos == nn.find("Mt2lestermutauFBInv75")) flcont=false;
//		if (string::npos == nn.find("AggregatedBins_17")) flcont=false;

	//	if (string::npos == nn.find("METFB_19")) flcont=false;
		if (string::npos == nn.find("nJets_12")) flcont=false;
		//if (string::npos == nn.find("_18") && string::npos == nn.find("_19")  && string::npos == nn.find("_20") && string::npos == nn.find("_21")) flcont=false;
		if (string::npos != nn.find("DeltaMET")) flcont=false;
//		if (string::npos == nn.find("BDTperBin_") || string::npos == nn.find("_15") && string::npos == nn.find("_16")  && string::npos == nn.find("_17")) flcont=false;
		//if (string::npos == nn.find("met_MT2lester_DZeta01J1D_15")) flcont=false;
		//if (string::npos == nn.find("BDTperBin_") || (string::npos == nn.find("0_17") && string::npos == nn.find("1_17") && string::npos == nn.find("2_17") && string::npos == nn.find("3_17") && string::npos == nn.find("4_17") && string::npos == nn.find("5_17") && string::npos == nn.find("6_17") && string::npos == nn.find("7_17") && string::npos == nn.find("8_17") && string::npos == nn.find("9_17"))) flcont=false;
		if (!flcont) continue;
...
if (SO_)
			...
			string fname= nextsource->GetName();
					...
					string wjet="WJets_";
				float normm = 1;
				if (!(std::string::npos != fname.find(qcd) || std::string::npos != fname.find(wjet)))	
				{ 
...
//cout << xsecc<<endl;
					float nGenn=1;
					if (std::string::npos == fname.find("QCD_DataDriven"))	nGenn = eventCountt->GetSumOfWeights();
//cout << "!!!!!"<<endl;

					normm = float(xsecc*Lumi) / float(nGenn)  ;
				}
					if (std::string::npos != fname.find(qcd) || std::string::npos != fname.find(wjet)) { normm =1.;}
					...
					if (key2) {
						...
						if (h2->GetSumOfWeights()<0.) h2->Scale(0);
			...
			|| std::string::npos != title_.find("W3JetsToLNu") || std::string::npos != title_.find("W4JetsToLNu") || std::string::npos != title_.find("WJets_"))  { col=mycolorwjet ; hwj->Add(h2); hwj->SetLineColor(col);}
		...
		if (std::string::npos != title_.find("ST_") || std::string::npos != title_.find("channel") )  {col= mycolorst; hstop->Add(h2);hstop->SetLineColor(col);}
			...
			if ( std::string::npos != title_.find("TTW") || std::string::npos != title_.find("TTZ") || std::string::npos != title_.find("tZq") || std::string::npos != title_.find("TG") || std::string::npos != title_.find("tG") || std::string::npos != title_.find("TTG") || std::string::npos != title_.find("ttW") || std::string::npos != title_.find("ttZ") || std::string::npos != title_.find("tZ") || std::string::npos != title_.find("TTT_") ) {col=mycolorttx ;httx->Add(h2);   httx->SetLineColor(col);}
						...
						if (!flagg)
						{
							...
							hs->Add(h2);
							allbkg->Add(h2);
//cout<<			((TH1D*)(hs->GetStack()->Last()))->GetEntries()<<" hs!!!!!!! "<<((TH1D*)(hs->GetStack()->Last()))->GetSumOfWeights()<<endl;
						}
...
*/
	...
	if (drawData)	pad2->Draw();
			...
			TH1D *hsum = ((TH1D*)(hs->GetStack()->Last())); // the "SUM"
      		        if ( string::npos == nn.find("CutFlowUnW") && string::npos == nn.find("1D_")) 
			hsum->GetXaxis()->SetRange(hh[1]->FindFirstBinAbove(1),hh[1]->FindLastBinAbove(MaxEventsBin));	
...
//cout <<hh[1]->FindFirstBinAbove(0) <<"  !!!!!!!!!!!! "<< endl;
//cout<< hh[1]->FindLastBinAbove(MaxEventsBin)<<endl;
...
if (SO_){
			...
			//	legend_c1->AddEntry(hh[3], "#tilde{#tau}_{L}(100,50)", "LEP");
			//	legend_c1->AddEntry(hh[4], "#tilde{#tau}_{L}(150,1)", "LEP");
			//	legend_c1->AddEntry(hh[5], "#tilde{#tau}_{L}(150,100)", "LEP");
			//	legend_c1->AddEntry(hh[6], "#tilde{#tau}_{L}(250,100)", "LEP");
			//	legend_c1->AddEntry(hh[7], "#tilde{#tau}_{L}(300,250)", "LEP");
				//legend_c1->AddEntry(hh[5], "C1N2(100,1)", "LEP");
			//	legend_c1->AddEntry(hh[7], "C1N2(100,75)", "LEP");
			//	legend_c1->AddEntry(hh[8], "C1N2(200,75)", "LEP");
...
if (!norm_) {
				...
				v2->SetLineColor(mycolorttx);v2->SetFillColor(mycolorttx);legend_c1->AddEntry(v2, "TTX/TG", "f");
				...
				v5->SetLineColor(mycolorqcd);v5->SetFillColor(mycolorqcd);legend_c1->AddEntry(v5, "QCD", "f");
...
if (norm_) {v1->SetFillColor(0);v1->SetLineColor(kRed);legend_c1->AddEntry(v1, "BKG", "LEP");}
			...
			hsum->Draw("hist");
cout<<			hsum->GetEntries()<<" hsum!!!!!!! "<<hsum->GetSumOfWeights()<<endl;
			if (!norm_)
				hs->Draw("same hist");
...
if (SO_){
			for (int ij=2;ij<2;ij++){
				...
				if (norm_) { hh[ij]->Scale(1/hh[ij]->Integral());/*hh[ij]->SetLineColor(kGreen+ij);*/}
...
void ModifyHist (TH1D* &h, int cl_ ,float & lumi,float & weight,string & title_, bool norm_=false,float XSec=1.)
		{
			...
			if (std::string::npos != title_.find("wJets")|| std::string::npos != title_.find("WJetsToLNu") || std::string::npos != title_.find("W1JetsToLNu") || std::string::npos != title_.find("W2JetsToLNu") 
			|| std::string::npos != title_.find("W3JetsToLNu") || std::string::npos != title_.find("W4JetsToLNu") || std::string::npos != title_.find("WJets_"))  { col=mycolorwjet ; }
