...
void OverlapWJNominal()
{
	...
	string fout = "WJets_Nominal_B.root";
...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	while ((key = (TKey *) nextkey ())) {
		...
		//if (string::npos == nn.find("BDTmutau_stau100_LSP50My_16")) flcont=false;
		...
		c1->SetTitle(obj->GetName());
			...
			cout<<"  Will add "<<cl<<"  "<<h1->GetName()<<"  "<<h1->GetTitle()<<"  "<<h1->GetName()<<"  "<<h1->GetSumOfWeights()<<endl;
			...
			while (nextsource) {
				...
				TKey *keyTemp;
				TString Kname = h1->GetName();	
				TString Kname2;							
				
				if (key2) {
					cl++;
					iterator++;
//cout << "IIIIIIIIIII iterator "<< iterator<<endl;
					TH1D *h2;
					Kname2 = Kname;
					if (iterator==2) {Kname2 = Kname.Replace(Kname.Length()-1,1,"7");}
					if (iterator==3) Kname2 = Kname.Replace(Kname.Length()-1,1,"8");
					cout <<"Kname2  " << Kname2<<endl;
					keyTemp = key2;
					if (iterator==2 || iterator==3) keyTemp = (TKey *) gDirectory->GetListOfKeys ()->FindObject (Kname2);

					h2 = (TH1D*) keyTemp->ReadObj ();
					h2->SetLineWidth(4);
					ModifyHist (h2, cl,Lumi,lumiweights[cl-1],titles[cl-1],norm_);
					h2->SetStats(0);
					hh[(int)cl] = h2;

			//cout<<"  Will add "<<cl<<"  "<<fname<<"  "<<h1->GetName()<<"  "<<h1->GetTitle()<<"  "<<h2->GetName()<<"  "<<h2->GetSumOfWeights()<<endl;
		...
		//cout<<" allbkgD!!!!!!!!!!!!!!!!!!!!!1 "<< allbkgD->GetSumOfWeights() <<endl;
		//cout<<" allRegD!!!!!!!!!!!!!!!!!!!!!1 "<< allRegD->GetSumOfWeights() <<endl;
							}
					}
		...
		cout<<" A "<< allRegA->GetSumOfWeights() <<"  purity of A "<<float( (allRegA->GetSumOfWeights()-allbkgA->GetSumOfWeights())/allRegA->GetSumOfWeights())  <<" C "<< allRegC->GetSumOfWeights() <<" D "<< allRegD->GetSumOfWeights() <<" purity of C  "<<float( (allRegC->GetSumOfWeights()-allbkgC->GetSumOfWeights())/allRegC->GetSumOfWeights())<<"  purity of D "<<float( (allRegD->GetSumOfWeights()-allbkgD->GetSumOfWeights())/allRegD->GetSumOfWeights())<<endl;

		cout<<" allbkgA "<< allbkgA->GetSumOfWeights() <<" allbkgC "<< allbkgC->GetSumOfWeights() <<" allbkgD "<< allbkgD->GetSumOfWeights() <<endl;
	...
	if ( string::npos != nn.find("met_MT2lester_DZeta01J1D")  ) ratioCD = double(nc/nd);
	if ( string::npos == nn.find("AggregatedBins")  ) allRegA->Scale(double(nc/nd));
	if ( string::npos != nn.find("AggregatedBins")  ) allRegA->Scale(ratioCD);
				...
				target->cd ("mutau");
				TString Hname = allRegA->GetName();
				Hname = Hname.Replace(Hname.Length()-1,1,"5");
				allRegA->SetNameTitle(Hname,Hname);
