...
import os
import ROOT as r
import plottery_utils as utils
...
class Options(object):
    ...
    def __init__(self, options={}, kind=None):
        ...
        self.recognized_options = {
            ...
            "legend_border": { "type": "Boolean", "desc": "show legend border?", "default": False, "kinds": ["1dratio","graph"], },
            ...
            "legend_datalabel": { "type": "String", "desc": "label for the data histogram in the legend", "default": "Observed", "kinds": ["1dratio"], },
            ...
            # Ratio
            "ratio_name": { "type": "String", "desc": "name of ratio pad", "default": "Obs. / Pred.", "kinds": ["1dratio"], },
...
#        utils.move_in_overflows(data)
    ...
    if syst:
        ...
        bgs_syst.SetLineWidth(0)
        legend.AddEntry(bgs_syst,"Bkg. uncertainty","f")
...
#        map(utils.move_in_overflows, sigs)
        colors = cycle([r.kBlue, r.kRed, r.TColor.GetColor("#66ccc3"), r.kTeal-5])
        if len(sig_labels) < len(sigs):
            sig_labels = [sig.GetTitle() for sig in sigs]
	iC = 0
        for hsig,signame,color in zip(sigs, sig_labels,colors):
            if (iC == 0) : hsig.SetMarkerStyle(43) # 2 has errors
            if (iC == 1) : hsig.SetMarkerStyle(33) # 2 has errors
            if (iC == 2) : hsig.SetMarkerStyle(34) # 2 has errors
	    ...
	    #hsig.SetLineStyle(2)
	    if(iC == 0) : hsig.SetLineStyle(2)
	    if(iC == 1) : hsig.SetLineStyle(3)
	    if(iC == 2) : hsig.SetLineStyle(6)
	    #hsig.SetLineStyle(9)
            legend.AddEntry(hsig,signame, "L")
            hsig.Draw("samehist")
	    iC = iC+1
    ...
    if do_ratio:
        ...
        if opts["ratio_numden_indices"]:
            ...
            denom.Reset()
            denom = sum(bgs,denom)
#        utils.move_in_overflows(numer)
#        utils.move_in_overflows(denom)
        ...
        if opts["ratio_pull"]:
            for ibin in range(-1,ratio.GetNbinsX()+3):
                ...
                # gaussian pull
                pull = (ratio_val-1.)/((numer_err**2.+denom_err**2.)**0.5+.000001)
                if numer_val > 1e-6:
                    # more correct pull, but is inf when 0 data, so fall back to gaus pull in that case
                    pull = r.RooStats.NumberCountingUtils.BinomialObsZ(numer_val,denom_val+.0000001,denom_err/(denom_val+.000001));
...
#        utils.move_in_overflows(ratio)
#	for ibin in range(-1,ratio.GetNbinsX()+3):
#		print(bin,ratio.GetBinContent(ibin),ratio.GetBinError(ibin),ratio.GetBinCenter(ibin))
        ratio.GetYaxis().CenterTitle(True)
        ratio.Draw("axis")
	#r.gStyle.SetOptStat(000101111)
#    	utils.move_in_overflows(ratio_syst)
        if syst and not opts["ratio_pull"]:
            ratio_syst.Draw("E2 SAME")
	x1,y1,x2,y2=.3,.6,.6,.8

    	legend2 = r.TLegend(x1,y1,x2,y2)
        legend2.SetBorderSize(0)
        legend2.AddEntry(ratio_syst, "Stat. + syst. uncertainty" , "f")
        ...
        line = r.TLine()
        line.SetLineColor(r.kBlack); #r.kRed);#Gray+2);
        line.SetLineWidth(1); #2);
        line.SetLineStyle(1); #9)
        ...
        #legend2.AddEntry(ratio, "stat+syst" , "E")
        #legend2.Draw()
        ...
        if opts["ratio_chi2prob"] or (opts["ratio_pull"] and opts["ratio_pull_numbers"]):
            ...
            if opts["ratio_pull"] and opts["ratio_pull_numbers"]:
		#print ratio
		#rat_trimmed = ratio[ratio < 5]
		ratioClone = ratio.Clone("ratioClone")
		for bin_ in range(1,ratio.GetNbinsX()+1):
		  print bin_,ratio.GetBinContent(bin_)
		  if (np.abs(ratio.GetBinContent(bin_)) > 2):
			ratioClone.SetBinContent(bin_,0)
                mean, sigma, vals = utils.get_mean_sigma_1d_yvals(ratioClone)
