...
#file ="Var_BDTmutau_Stau100_16_35invfb_mt_SR.root"
	...
	#hS1=f1.Get("Stau125_LSP1_left_B_"+tag)
	...
	if('METFB' in tag) : 
	  ...
	  xaxis = 'BDT(150) Discriminant'
	elif('Stau200' in tag) :
	  xaxis = 'BDT(200) Discriminant'
	...
	y-=1
	  ...
	  print(bin_,y,bin_+x,b7.GetBinContent(y)+hS1.GetBinContent(bin_+x))
	...
	data = b6,
	sigs = [b7,b8,b9],
	#sig_labels = ['#tilde{#tau}_{L}(125), #tilde{#chi}_{1}^{0}(1)'],#,'#tilde{#tau}_{L}(150), #tilde{#chi}_{1}^{0}(1)','#tilde{#tau}_{L}(200), #tilde{#chi}_{1}^{0}(1)'],
	sig_labels = ['#tilde{#tau}_{L}(100), #tilde{#chi}_{1}^{0}(1)','#tilde{#tau}_{L}(125), #tilde{#chi}_{1}^{0}(1)','#tilde{#tau}_{L}(200), #tilde{#chi}_{1}^{0}(1)'],
	colors = colors,
	...
	options = {
	    ...
	    "extra_text_xpos" : .8 ,
	    "extra_text_ypos" : .525,
	    #"legend_coordinates" : [.0,.65,.95,1.],
	    "legend_coordinates" : [.0,.6,.95,1.],
	    "show_bkg_errors" :False,
	    "yaxis_range" : [.5,200000000],
	    "ratio_range":[.4,2.5],
	    "hist_disable_xerrors": True,
	    "output_name": "plots/"+file.replace(".root",""),
