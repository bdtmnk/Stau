...
void TableForUnrolledDistrSyst(TString directory = "",
	  ...
	  TString File = "Var_BDTmutau_Stau150_16_35invfb_mt_SR",
	  string varN  = "Var_BDTmutau_Stau150_16_35invfb_mt_SR",
	  TString variable = "",
	  TString Suffix = "Syst",
	  TString xtitle = "",
...

{
	  ...
	  vars_.push_back("BDTmutau_Stau150_16");
...
for (int syst=0;syst<syst_.size();++syst)
{
	...
	cout<<" Will try now to open "<<file->GetName()<<" for syst "<<syst<<endl;
     ...
     Signal[syst] = (TH1D*)file->Get("Stau150_LSP1_left_B_"+Variable);
