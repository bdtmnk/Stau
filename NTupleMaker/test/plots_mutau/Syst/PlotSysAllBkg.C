...
#
using namespace std;
void PlotSysAllBkg(TString fileName = "Templ_met_MT2lester_DZeta01J1D_17_35invfb_mt_C1N2_SR_CR1_out.root",
	     TString histName = "total_background",
	     //TString histName = "data_obs",
	   //  TString histName = "1D_C1N2_100_LSP1_B_met_MT2lester_DZeta01J1D_17",
	     TString dirName  = "SR_CR1",
	     TString sysName  = "",
	     TString header = "",
	     TString SysLeg = "AllBkg",
	     float xmax = 100,
	     float upRange = -100) {
...
//    if (histNominal->GetBinContent(iB)>0){
 ...
 cout<<" Bin  "<<iB<<" nom  "<<histNominal->GetBinContent(iB)<<" plus  "<<histUp->GetBinContent(iB)<<" fr "<<fabs((histNominal->GetBinContent(iB)-histUp->GetBinContent(iB))/histNominal->GetBinContent(iB))<<"  min  "<<histDown->GetBinContent(iB)<<" fr "<<fabs((histNominal->GetBinContent(iB)-histDown->GetBinContent(iB))/histNominal->GetBinContent(iB))<<"  max deviation  "<<max( fabs( (histNominal->GetBinContent(iB)-histUp->GetBinContent(iB))/histNominal->GetBinContent(iB)), fabs((histNominal->GetBinContent(iB)-histDown->GetBinContent(iB))/histNominal->GetBinContent(iB)))<<endl;
...
/*
  ...
  TString textfilenamee = "systematics_WW_misBTag.txt";
...
cout<<" max Relative diff WW misBTag " <<std::max(   fabs((dNom-dUp)/dNom), fabs((dNom-dDown)/dNom)) <<endl;
 ...
 // canv1->Print("figures/systematics/"+dirName+"_"+histName+"_"+sysName+"_sys.C");
	...
	TString outname=sysName;
	 ofstream tfile;
        TString textfilename = "systematics_WW_misBTag.txt";
	tfile.open(textfilename);
				...
				float Downvar = (histNominal->GetBinContent(nb)-histDown->GetBinContent(nb)) / histNominal->GetBinContent(nb);
                                 ...
                                 //tfile <<"misBTag "<< histNominal->GetXaxis()->GetBinLabel(nb)<<" "<<Upvar<<"  "<<Downvar<<endl;
                                 tfile <<"misBTag "<< histNominal->GetXaxis()->GetBinLabel(nb)<<" "<<maxDev[nb]*100<<endl;

                                }
                                 tfile <<"Max deviation "<< cUpRel*100<<endl;
