...
#include <iostream> 
//WithoutNPV/
void PlotsForUnrolledDistrAllChannelsStau(TString directory = "",
	  TString suffix = "_SR_CR1_MuTau_all_wBinUncrt_PreFit",
...
//		BinLabels[i]=name;
    ...
    TH1D * fitHisto, *fitHisto2, *fitHistoPostH0, *fitHistoPostH1, *errPreFit, *errNormOnly,*errNormOnlyB, *errPreFit_tt,*errPreFit_wj,*errPreFit_dyj,*errPreFit_ztt,*errPreFit_sT,*errPreFit_vv,*errPreFit_ww,*errPreFit_ttx,*errPreFit_fakes,*errPreFit_rest;
    ...
    //filee = new TFile("/nfs/dust/cms/user/alkaloge/Workdir/CMSSW_7_4_7/src/Limits/STau/Results_SR_CR1/mlfit_met_MT2lester_DZeta01J1D_17_all.root","read");
    filee = new TFile("/nfs/dust/cms/user/alkaloge/Workdir/CMSSW_7_4_7/src/Limits/STau/Results_SR_CR1/mlfit_met_MT2lester_DZeta01J1D_18_mt_only.root","read");
    fileStat = new TFile("/nfs/dust/cms/user/alkaloge/TauAnalysis/new/new/StauAnalysis/New8025/CMSSW_8_0_25/src/DesyTauAnalyses/NTupleMaker/test/Processing/01Jets/MuTau/SR_CR1_MuTau-stau-stau_left.root","read");
...
//    fileStat = new TFile("/nfs/dust/cms/user/alkaloge/Workdir/CMSSW_7_4_7/src/Limits/STau/Results_SR_CR1/mlfit_all_normOnly.root","read");
    fitHisto = (TH1D*)filee->Get("shapes_fit_s/ch1/total_background");
    fitHisto2 = (TH1D*)filee->Get("shapes_fit_s/ch1/total_background");
    errPreFit = (TH1D*)fileStat->Get("tt_shape");
    errPreFit_tt = (TH1D*)fileStat->Get("tt_shape");
//    errPreFit_wj = (TH1D*)fileStat->Get("tt_shape");
    errPreFit_dyj = (TH1D*)fileStat->Get("dyj_shape");
    errPreFit_ztt = (TH1D*)fileStat->Get("ztt_shape");
    errPreFit_sT = (TH1D*)fileStat->Get("sT_shape");
//    errPreFit_vv = (TH1D*)fileStat->Get("tt_shape");
    errPreFit_ww = (TH1D*)fileStat->Get("ww_shape");
//    errPreFit_ttx = (TH1D*)fileStat->Get("ttx_shape");
    errPreFit_fakes = (TH1D*)fileStat->Get("fakes_shape");
    errPreFit_rest = (TH1D*)fileStat->Get("rest_shape");
//    errNormOnly = (TH1D*)fileStat->Get("shapes_prefit/ch1/total_background");

    nB = fitHisto->GetNbinsX();
    errPreFit->Reset();

    for (int nb=0;nb<=errPreFit->GetNbinsX()+1;++nb){

     errPreFit->SetBinContent(nb,errPreFit_tt->GetBinContent(nb)+errPreFit_dyj->GetBinContent(nb)+errPreFit_ztt->GetBinContent(nb)+errPreFit_sT->GetBinContent(nb)+errPreFit_ww->GetBinContent(nb)+errPreFit_fakes->GetBinContent(nb)+errPreFit_rest->GetBinContent(nb));
     errPreFit->SetBinError(nb,sqrt( errPreFit_tt->GetBinError(nb)*errPreFit_tt->GetBinError(nb) +  errPreFit_dyj->GetBinError(nb)*errPreFit_dyj->GetBinError(nb) + errPreFit_ztt->GetBinError(nb)*errPreFit_ztt->GetBinError(nb) + errPreFit_sT->GetBinError(nb)*errPreFit_sT->GetBinError(nb) + errPreFit_ww->GetBinError(nb)*errPreFit_ww->GetBinError(nb) + errPreFit_fakes->GetBinError(nb)*errPreFit_fakes->GetBinError(nb) + errPreFit_rest->GetBinError(nb)*errPreFit_rest->GetBinError(nb)));
    ...
    //errPreFit =  new TH1D ("","",nB,1,nB+1);
    TT =  new TH1D ("","",nB,1,nB+1);
    WJ =  new TH1D ("","",nB,1,nB+1);
    rest =  new TH1D ("","",nB,1,nB+1);
...
//    errNormOnlyB =  new TH1D ("","",nB,1,nB+1);
    ...
    for (int nb=1;nb <= fitHisto->GetNbinsX()+1; ++nb){ fitHistoPostH0->SetBinContent(nb,fitHisto->GetBinContent(nb)); fitHistoPostH0->SetBinError(nb,fitHisto->GetBinError(nb));}
    for (int nb=1;nb <= fitHisto2->GetNbinsX()+1; ++nb){ 
	    fitHistoPostH1->SetBinContent(nb,fitHisto2->GetBinContent(nb)); fitHistoPostH1->SetBinError(nb,fitHisto2->GetBinError(nb)); 
    	}
...
for (int svar=0;svar<vars_.size();++svar)
{
    ...
    //
    /*
    TH1D * WJq = (TH1D*)filee->Get("shapes_prefit/ch1/wj_"+Variable);
    TH1D * VVq = (TH1D*)filee->Get("shapes_prefit/ch1/dib_"+Variable);
    TH1D * TTXq = (TH1D*)filee->Get("shapes_prefit/ch1/ttx_"+Variable);
*/
    TH1D * restq = (TH1D*)filee->Get("shapes_prefit/ch1/rest_"+Variable);
    TH1D * TTq = (TH1D*)filee->Get("shapes_prefit/ch1/tt_"+Variable);
    TH1D * DYq = (TH1D*)filee->Get("shapes_prefit/ch1/dyj_"+Variable);
    TH1D * ZTTq = (TH1D*)filee->Get("shapes_prefit/ch1/ztt_"+Variable);
    TH1D * STq = (TH1D*)filee->Get("shapes_prefit/ch1/sT_"+Variable);
    TH1D * WWq = (TH1D*)filee->Get("shapes_prefit/ch1/ww_"+Variable);
    TH1D * QCDq = (TH1D*)filee->Get("shapes_prefit/ch1/fakes_"+Variable);
    TH1D * histSignal = (TH1D*)file->Get("stau-stau_left_200_LSP1_B_"+Variable);
    TH1D * histSignal2 = (TH1D*)file->Get("stau-stau_left_200_LSP10_B_"+Variable);
...
/*
 ...
 //   TH1D * histData = (TH1D*)file->Get("data_obs_"+Variable);
    TH1D * histData = (TH1D*)file->Get("data_obs");
    TH1D * histSignal = (TH1D*)file->Get("stau-stau_left_100_LSP1_B_"+Variable);
//    TH1D * histSignal2 = (TH1D*)file->Get("stau-stau_left_100_LSP50_B_"+Variable);
//    TH1D * histSignal3 = (TH1D*)file->Get("stau-stau_left_500_LSP100_B_"+Variable);
...
cout<<"         \\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|}  \\hline "<<endl;
cout<<"	 SR bin    &  TTJets     & Rest    & DYJets       & Fakes   & SingleT    & WW   &  Total Bkg ($\\pm total error)    & Observed \\\\"<<endl;
     cout<<" \\hline "<<endl;

     for (int b=1;b<allbkg->GetNbinsX()+1;++b){
float sumBin = errPreFit->GetBinContent(b);//TTcl->GetBinContent(b)+WJ->GetBinContent(b)+DY->GetBinContent(b)+ZTT->GetBinContent(b)+QCD->GetBinContent(b)+ST->GetBinContent(b)+VV->GetBinContent(b)+WW->GetBinContent(b)+TTX->GetBinContent(b);
    std::cout << std::setprecision(1);

    float total_err = errPreFit->GetBinError(b);
...
cout<<"	    \\end{table}"<<endl;
	...
	float ErrPost = eStatPost;//TMath::Sqrt(eStatPost*eStatPost+eLumiPost*eLumiPost + eElectronP*eElectronP + eMuonP*eMuonP + eTauP*eTauP + eMETP*eMETP + eTFRP*eTFRP + eLTFRP*eLTFRP + eTauIDP*eTauIDP);
      ...
      if (OverlayFit)  bkgdErr->SetBinError(iB,errPreFit->GetBinError(iB));
