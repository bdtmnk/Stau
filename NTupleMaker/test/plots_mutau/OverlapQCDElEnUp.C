...
Impose (TDirectory * target, TList * sourcelist, string & np_title_, vector<string> titles,vector<float> xsecs)
{
	...
	path.Remove (0, 2);
				...
				TFile *foutA = TFile::Open ("QCD_ElEnUp_A.root", "RECREATE");
				TFile *foutC = TFile::Open ("QCD_ElEnUp_C.root", "RECREATE");
				TFile *foutD = TFile::Open ("QCD_ElEnUp_D.root", "RECREATE");
	...
	ofstream tfile;
	TFile * filee = new TFile("QCD_ElEnUp_B.root","update");
	filee->cd("mutau");
	//target->ls();
