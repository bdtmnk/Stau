...
//                                               "!V:!Silent:Color:DrawProgressBar:Transformations=I;D;P;G,D:AnalysisType=Classification" );
   ...
   //dataloader->AddVariable( "mass", "stau mass", "GeV", 'F' );
...
//TH1D *hMt2lestermutau = new TH1D ("Mt2lestermutauFB_","Mt2lestermutau ",100,2,302);
	...
	string line;
		...
		if (std::string::npos != HHname.find(stau) && Sxsec < 0.9) {name2 = WorkDir+channel+"/"+Signal+name+".root";xs=Sxsec*fact;}
       		cout << i << endl;
       		cout << name << endl; 
		name3 = name;
		//treeName = "tree"+name;
...
//cout <<"V1 and V2  !!!   "<< LeptV1.Pt() <<"  "<< 	LeptV2.Pt() <<"  "<< TauV.Pt()<<endl;		
		...
		bool isfakeMuFromLFQ =  (  genLeptonMatchedLFQ >0.5 && !isData) ;			
			...
			if (std::string::npos != HHname.find(stau)  && ta_IsoFlagTight[0] < 0.5 ) continue;
...
/*
 ...
 {
        if (LeptV2.Pt()>20 && LeptV2.Pt()<30) all_weights *= 0.338533;
         if (LeptV2.Pt()>30 && LeptV2.Pt()<40) all_weights *= 0.324686;
         if (LeptV2.Pt()>40 ) all_weights *= 0.295238;
 }
 if (  fabs(LeptV2.Eta()) > 0.8 && fabs(LeptV2.Eta()) < 1.44 )
 {
        if (LeptV2.Pt()>20 && LeptV2.Pt()<30) all_weights *= 0.342852;
         if (LeptV2.Pt()>30 && LeptV2.Pt()<40) all_weights *= 0.325891;
         if (LeptV2.Pt()>40 ) all_weights *= 0.28947;
 }
 if (  fabs(LeptV2.Eta()) > 1.44 && fabs(LeptV2.Eta()) < 1.566 )
 {
        if (LeptV2.Pt()>20) all_weights *= 0.372907;
 }
 if (  fabs(LeptV2.Eta()) > 1.566 && fabs(LeptV2.Eta()) < 2.3 )
 {
        if (LeptV2.Pt()>20 && LeptV2.Pt()<30) all_weights *= 0.388045;
         if (LeptV2.Pt()>30 && LeptV2.Pt()<40) all_weights *= 0.370063;
         if (LeptV2.Pt()>40 ) all_weights *= 0.341425;
 }
}*/
...
//if (ta_IsoFlagTight[0] < 0.5) continue;
			...
			if (std::string::npos != HHname.find(stau))
				...
				//dataloader->AddSignalTrainingEvent( vars, 0.25*MCweight*all_weights );
           			else if (std::string::npos != HHname.find("left")) {
						if (LeptV2.Pt() > 30 && ta_IsoFlagVTight[0] > 0.5)	dataloader->AddSignalTestEvent( vars, MCweight*all_weights );}
