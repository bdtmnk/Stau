...
int myTMVA( TString myMethodList = "", 
		...
		float EntriesProcTest = 0.5, /// from EntriesProc!!!!!
...
//TH1D *hMt2lestermutau = new TH1D ("Mt2lestermutauFB_","Mt2lestermutau ",100,2,302);
  ...
  treeName->SetBranchAddress("npartons",&npartons,&b_npartons);
		...
		if (std::string::npos != HHname.find(stau)) EntriesProc=0.8;
   ...
   //if (retval < 0.75) continue;
			...
			if (std::string::npos != HHname.find(stau))
				...
				//dataloader->AddSignalTrainingEvent( vars, 0.25*MCweight*all_weights );
           			//else if (std::string::npos != HHname.find("Stau100")) {dataloader->AddSignalTestEvent( vars, MCweight*all_weights );}
