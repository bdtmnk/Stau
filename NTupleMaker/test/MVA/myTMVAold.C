...
int myTMVAold( TString myMethodList = "", 
		TString outfileName = "TMVA_stau-stau150_LSP1My_mutau_SignSkipNorm.root", 
		string BDTname = "mutau",
		float Sxsec = 1,
		TString Signal = "stau-stau",  
		TString channel = "mutau", 
		TString WorkDir = "/nfs/dust/cms/user/guezelgc/disappearingTracks/stau/CMSSW_9_2_13/src/DesyTauAnalyses/NTupleMaker/test/",
...
//                                               "!V:!Silent:Color:DrawProgressBar:Transformations=I;D;P;G,D:AnalysisType=Classification" );
   ...
   dataloader->AddVariable( "Lept1Pt", "Lept1Pt", "", 'F' );
   dataloader->AddVariable( "Lept2Pt", "Lept2Pt", "units", 'F' );
   dataloader->AddVariable( "EtaDil", "EtaDil", "units", 'F' );
   dataloader->AddVariable( "MTtot", "MTtot", "units", 'F' );
   dataloader->AddVariable( "Dzeta", "Dzeta", "units", 'F' );
   dataloader->AddVariable( "Minv", "Minv", "units", 'F' );
   dataloader->AddVariable( "dR", "dR", "units", 'F' );
   dataloader->AddVariable( "MT", "MT", "units", 'F' );
   dataloader->AddVariable( "MCTb", "MCTb", "units", 'F' );
   dataloader->AddVariable( "MT2lester", "MT2lester", "units", 'F' );
   dataloader->AddVariable( "dMETPhiL1", "dMETPhiL1", "units", 'F' );
   dataloader->AddVariable( "dMETPhiL2", "dMETPhiL2", "units", 'F' );


/*
   dataloader->AddVariable( "mu_eta", "mu_eta", "units", 'F' );
   dataloader->AddVariable( "tau_eta", "tau_eta", "units", 'F' );
   dataloader->AddVariable( "dEtaJMu", "dEtaJMu", "units", 'F' );
   dataloader->AddVariable( "dEtaJTau", "dEtaJTau", "units", 'F' );
   dataloader->AddVariable( "mu_dz", "mu_dz", "units", 'F' );
   dataloader->AddVariable( "mu_dxy", "mu_dxy", "units", 'F' );
   dataloader->AddVariable( "dPhiDiL", "dPhiDiL", "units", 'F' );
   dataloader->AddVariable( "dR_taujet", "dR_taujet", "units", 'F' );*/
/*
   dataloader->AddVariable( "HTOsqrMET", "HTOsqrMET", "units", 'F' );
   dataloader->AddVariable( "PtJ0", "PtJ0", "units", 'F' );
*/
/*
   dataloader->AddVariable( "nEl", "nEl", "units", 'I' );
   dataloader->AddVariable( "nTau", "nTau", "units", 'I' );
...
TH1D *hMt2lestermutau = new TH1D ("Mt2lestermutauFB_","Mt2lestermutau ",100,2,302);
	...
	string line;
		...
		name2 = WorkDir+channel+"2016/"+name+"_B_OS.root";
		string HHname = name;	
		if (std::string::npos != HHname.find(stau)) name2 = WorkDir+channel+"2016/"+name+".root";
		if (std::string::npos != HHname.find(stau) && Sxsec < 0.9) {name2 = WorkDir+channel+"2016/"+Signal+".root"; name=Signal;HHname=Signal;xs=Sxsec;}
       		cout << i << endl;
       		cout << name << endl; 
		name3 = name;
		//treeName = "tree"+name;
  ...
  treeName->SetBranchAddress("npartons",&npartons,&b_npartons);
		...
		for (UInt_t i=0; i<(EntriesProc*treeName->GetEntries()); i++) {
			...
			METV.SetPx(met_ex);
			METV.SetPy(met_ey);
			float (met_ex*met_ex + met_ey*met_ey); 
			if (std::string::npos != HHname.find(WJ) || std::string::npos != HHname.find(DY)) 
			{
				...
				float met=sqrt(met_ex_recoil*met_ex_recoil+met_ey_recoil*met_ey_recoil);
			}
			...
			if (std::string::npos != HHname.find(stau))
				{
           			...
           			if (i < (EntriesProc*treeName->GetEntries())*(1-EntriesProcTest)) dataloader->AddSignalTrainingEvent( vars, MCweight*all_weights/0.24 );
				...
				{
           			if (i < (EntriesProc*treeName->GetEntries())*(1-EntriesProcTest)) dataloader->AddBackgroundTrainingEvent( vars, MCweight*all_weights/0.24 );
...
*/
   ...
   if (Use["BDT"+BDTname])  // Adaptive Boost
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT"+BDTname,
                           "!H:!V:NTrees=850:MinNodeSize=2.5%:MaxDepth=3:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=SDivSqrtSPlusB:nCuts=20:SkipNormalization=true" );
