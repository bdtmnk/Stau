...
//  const string TauFakeRateFile = cfg.get<string>("TauFakeRateEff");
  ...
  FakeList.clear();
  FakeList.push_back("Unknown");
  FakeList.push_back("#tau_{h}, #tau #rightarrow l");
  FakeList.push_back("#tau_{h}, Prompt l");
  FakeList.push_back("#tau_{h}, G #rightarrow l");
  FakeList.push_back("#tau_{h}, HFQ #rightarrow l");
  FakeList.push_back("#tau_{h}, LFQ #rightarrow l");
  FakeList.push_back("J #rightarrow #tau, #tau #rightarrow l");
  FakeList.push_back("J #rightarrow #tau, Prompt l");
  FakeList.push_back("J #rightarrow #tau, G #rightarrow l");
  FakeList.push_back("J #rightarrow #tau, HFQ #rightarrow l");
  FakeList.push_back("J #rightarrow #tau, LFQ #rightarrow l");
  FakeList.push_back("#mu #rightarrow #tau, #tau #rightarrow l");
  FakeList.push_back("#mu #rightarrow #tau, Prompt l");
  FakeList.push_back("#mu #rightarrow #tau, G #rightarrow l");
  FakeList.push_back("#mu #rightarrow #tau, HFQ #rightarrow l");
  FakeList.push_back("#mu #rightarrow #tau, LFQ #rightarrow l");
  FakeList.push_back("e #rightarrow #tau, #tau #rightarrow l");
  FakeList.push_back("e #rightarrow #tau, Prompt l");
  FakeList.push_back("e #rightarrow #tau, G #rightarrow l");
  FakeList.push_back("e #rightarrow #tau, HFQ #rightarrow l");
  FakeList.push_back("e #rightarrow #tau, LFQ #rightarrow l");
  FakeList.push_back("G #rightarrow #tau, !Prompt l");
  FakeList.push_back("#mu #rightarrow #tau, !Prompt l");
  FakeList.push_back("e #rightarrow #tau, !Prompt l");
  FakeList.push_back("");
  ...
  FakeListJet.clear();
  FakeListJet.push_back("G #rightarrow #tau, #tau #rightarrow l");
  FakeListJet.push_back("G #rightarrow #tau, Prompt l");
  FakeListJet.push_back("G #rightarrow #tau, G #rightarrow l");
  FakeListJet.push_back("G #rightarrow #tau, HFQ #rightarrow l");
  FakeListJet.push_back("G #rightarrow #tau, LFQ #rightarrow l");
  FakeListJet.push_back("G #rightarrow #tau, !Prompt l");
  FakeListJet.push_back("LFQ #rightarrow #tau, #tau #rightarrow l");
  FakeListJet.push_back("LFQ #rightarrow #tau, Prompt l");
  FakeListJet.push_back("LFQ #rightarrow #tau, G #rightarrow l");
  FakeListJet.push_back("LFQ #rightarrow #tau, HFQ #rightarrow l");
  FakeListJet.push_back("LFQ #rightarrow #tau, LFQ #rightarrow l");
  FakeListJet.push_back("LFQ #rightarrow #tau, !Prompt l");
  FakeListJet.push_back("HFQ #rightarrow #tau, #tau #rightarrow l");
  FakeListJet.push_back("HFQ #rightarrow #tau, Prompt l");
  FakeListJet.push_back("HFQ #rightarrow #tau, G #rightarrow l");
  FakeListJet.push_back("HFQ #rightarrow #tau, HFQ #rightarrow l");
  FakeListJet.push_back("HFQ #rightarrow #tau, LFQ #rightarrow l");
  FakeListJet.push_back("HFQ #rightarrow #tau, !Prompt l");
  FakeListJet.push_back("");
...
if (!CutBasedTauId){
   ...
   //isoTau = analysisTree.tau_chargedIsoPtSum[tIndex];
 	...
 	if ((int)tau_index>-1 && analysisTree.tau_byLooseIsolationMVArun2v1DBoldDMwLT[tau_index] > 0.5 && analysisTree.tau_byTightIsolationMVArun2v1DBoldDMwLT[tau_index] < 0.5) isLoose  = true;
...
if ((!isW && !isDY) && !isData) 
	...
	for (unsigned int gt = 0 ; gt < analysisTree.gentau_count; ++gt){
	  ...
	  if (Drr < 0.5 && analysisTree.gentau_isPrompt[gt] > 0.5  && genTauV.Pt() > 15. ) isTauMatched = true;
...
//cout<<analysisTree.genparticles_pdgid[igen]<<" isDirectHadronDecayProduct "<<analysisTree.genparticles_isDirectHadronDecayProduct[igen]<<" isTauDecay "<<analysisTree.genparticles_isTauDecayProduct[igen]<<" isDirectTau  "<<analysisTree.genparticles_isDirectTauDecayProduct[igen]<<"  isPrompt  "<<analysisTree.genparticles_isPrompt[igen]<<"  isDecayeLepton "<<analysisTree.genparticles_isDecayedLeptonHadron[igen]<<" is there gentau matched ?  "<<genTauMatched<<endl;
		...
		if (DrTauLepton < 0.5 && genLepV.Pt() > 8. ) 
		...
		bool isfakeTauFromGluon = matchedTauToGluon>0.5 ;
		bool isfakeTauFromLFQ = isfakeTauFromJet && matchedTauToLFQ>0.5 ;
		bool isfakeTauFromHFQ = isfakeTauFromJet && matchedTauToHFQ>0.5 ;
		...
		int category = 0;
		int categoryJet = 0;
		if (isData) category = 0;
		if (isData) categoryJet = 0;

int cJ = 0;
		if (!isRealTau) {

	if (!isfakeTauFromGluon){
		if (isfakeTauFromJet && isMuFromTau) category = 6;
		if (isfakeTauFromJet && isMuPromptLepton) category = 7;
		if (isfakeTauFromJet && isfakeMuFromGluon) category = 8;
		if (isfakeTauFromJet && isfakeMuFromHFQ) category = 9;
		if (isfakeTauFromJet && isfakeMuFromLFQ) category = 10;
		}

		if (isfakeTauFromGluon && isMuFromTau) category = 1;
		if (isfakeTauFromGluon && isMuPromptLepton) category = 2;
		if (isfakeTauFromGluon && isfakeMuFromGluon) category = 3;
		if (isfakeTauFromGluon && isfakeMuFromHFQ) category = 4;
		if (isfakeTauFromGluon && isfakeMuFromLFQ) category = 5;

if (isfakeTauFromGluon) cJ=0;
if (isfakeTauFromLFQ) cJ=6;
if (isfakeTauFromHFQ) cJ=12;

		if (isfakeTauFromGluon && isMuFromTau) categoryJet = cJ;
		if (isfakeTauFromGluon && isMuPromptLepton) categoryJet = 1+cJ;
		if (isfakeTauFromGluon && isfakeMuFromGluon) categoryJet = 2+cJ;
		if (isfakeTauFromGluon && isfakeMuFromHFQ) categoryJet = 3+cJ;
		if (isfakeTauFromGluon && isfakeMuFromLFQ) categoryJet = 4+cJ;
		if (isfakeTauFromGluon && !isMuFromTau && !isMuPromptLepton && !isfakeMuFromGluon && !isfakeMuFromHFQ && !isfakeMuFromLFQ)  categoryJet = 5 +cJ;


		if (isfakeTauFromMu && isMuFromTau) category = 11;
		if (isfakeTauFromMu && isMuPromptLepton) category = 12;
		if (isfakeTauFromMu && isfakeMuFromGluon) category = 13;
		if (isfakeTauFromMu && isfakeMuFromHFQ) category = 14;
		if (isfakeTauFromMu && isfakeMuFromLFQ) category = 15;

		if (isfakeTauFromEl && isMuFromTau) category = 16;
		if (isfakeTauFromEl && isMuPromptLepton) category = 17;
		if (isfakeTauFromEl && isfakeMuFromGluon) category = 18;
		if (isfakeTauFromEl && isfakeMuFromHFQ) category = 19;
		if (isfakeTauFromEl && isfakeMuFromLFQ) category = 20;


		if (isfakeTauFromGluon && !isMuFromTau && !isMuPromptLepton && !isfakeMuFromGluon && !isfakeMuFromHFQ && !isfakeMuFromLFQ)  category = 21;
		if (isfakeTauFromMu && !isMuFromTau && !isMuPromptLepton && !isfakeMuFromGluon && !isfakeMuFromHFQ && !isfakeMuFromLFQ)  category = 22;
		if (isfakeTauFromEl && !isMuFromTau && !isMuPromptLepton && !isfakeMuFromGluon && !isfakeMuFromHFQ && !isfakeMuFromLFQ)  category = 23;
...
//	if (isTauMatched && !isGenLeptonMatched) weight *= 0.9;
 ...
 //     cout<<   " found the pt "<<ptProbe<<"  "<<analysisTree.tau_pt[tau_loose]<<" "<<ptBin<<"  "<<absEtaProbe<<"  "<<etaBin<<endl;
      ...
      if (!isTauMatched && isWJ  && !isData) continue;
      if (isfakeTauFromPromptLepton && !isData) continue;
