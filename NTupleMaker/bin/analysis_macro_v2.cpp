...
*/
  ...
  const string Muon17TriggerFile = cfg.get<string>("Muon17Trigger");
  const string Muon8TriggerFile = cfg.get<string>("Muon8Trigger");

  string cmsswBase = (getenv ("CMSSW_BASE"));
  string fullPathToJsonFile = cmsswBase + "/src/DesyTauAnalyses/NTupleMaker/test/json/" + jsonFile;
 

  ScaleFactor * SF_DataMu17 = new ScaleFactor();
  ScaleFactor * SF_DataMu8 = new ScaleFactor();
  ScaleFactor * SF_MCMu17 = new ScaleFactor();
  ScaleFactor * SF_MCMu8 = new ScaleFactor();

  SF_DataMu17->init_ScaleFactor(TString(cmsswBase)+"/src/"+TString(Muon17TriggerFile));
  SF_DataMu8->init_ScaleFactor(TString(cmsswBase)+"/src/"+TString(Muon8TriggerFile));
  SF_MCMu17->init_ScaleFactor(TString(cmsswBase)+"/src/"+TString(Muon17TriggerFile));
  SF_MCMu8->init_ScaleFactor(TString(cmsswBase)+"/src/"+TString(Muon8TriggerFile));
...
///////////////////////////////////////////////////////////////////////////// systematic study end
   ...
   isDZFilter = true;
	...
	//applying SF
     float  pt_1 = analysisTree.muon_pt[iLeading];
     float  eta_1 = analysisTree.muon_eta[iLeading];
     float  phi_1 = analysisTree.muon_phi[iLeading];
     float  pt_2 = analysisTree.muon_pt[iTrailing];
     float  eta_2 = analysisTree.muon_eta[iTrailing];
     float  phi_2 = analysisTree.muon_phi[iTrailing];
      

    float Mu17EffDataLead = 1;
    float Mu8EffDataLead = 1;
    float Mu17EffMCLead = 1;
    float Mu8EffMCLead = 1;
    float Mu17EffDataTrail = 1;
    float Mu8EffDataTrail = 1;
    float Mu17EffMCTrail = 1;
    float Mu8EffMCTrail = 1;
...
//      Lumi = 36590.; LumiA = 20233.; LumiB = 16357.;
      ...
      Mu17EffDataLead = (float)SF_DataMu17->get_EfficiencyData(double(pt_1),double(eta_1));
      Mu8EffDataLead = (float)SF_DataMu8->get_EfficiencyData(double(pt_1),double(eta_1));

      Mu17EffMCLead = (float)SF_MCMu17->get_EfficiencyData(double(pt_1),double(eta_1));
      Mu8EffMCLead = (float)SF_MCMu8->get_EfficiencyData(double(pt_1),double(eta_1));

      Mu17EffDataTrail = (float)SF_DataMu17->get_EfficiencyData(double(pt_2),double(eta_2));
      Mu8EffDataTrail = (float)SF_DataMu8->get_EfficiencyData(double(pt_2),double(eta_2));

      Mu17EffMCTrail = (float)SF_MCMu17->get_EfficiencyData(double(pt_2),double(eta_2));
      Mu8EffMCTrail = (float)SF_MCMu8->get_EfficiencyData(double(pt_2),double(eta_2));


	float termAD =0;   termAD = (Mu17EffDataLead*Mu8EffDataTrail) ; 
	float termBD =0;   termBD = (Mu17EffDataTrail*Mu8EffDataLead)  ; 
	float termCD =0;   termCD = (Mu17EffDataLead*Mu8EffDataLead);

	float termAMC =0;  termAMC = (Mu17EffMCLead*Mu8EffMCTrail) ; 
	float termBMC =0;  termBMC = (Mu17EffMCTrail*Mu8EffMCLead) ; 
	float termCMC =0;  termCMC = (Mu17EffMCLead*Mu8EffMCLead);

	 float trigWeightD = termAD + termBD - termCD;
	 float trigWeightMCD = termAMC + termBMC - termCMC;


	weight *= trigWeightD/trigWeightMCD;
