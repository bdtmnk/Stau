...
#include "DesyTauAnalyses/NTupleMaker/interface/AnalysisMacroHZZ.h"
...
int main(int argc, char * argv[]) {
    ...
    const double dzTauCut = cfg.get<double>("dzTauCut"); 
    //const double dRleptonCutTau = cfg.get<double>("dRleptonCutTau"); 
    ...
    */	
    ...
    for (int iF=0; iF<nTotalFiles; ++iF) {
        ...
        //This is where ALL the info comes from... the AC1B is the analysis tree with all the MC or all the Data
        TTree * _tree = NULL;
        ...
        for (Long64_t iEntry=0; iEntry<numberOfEntries; ++iEntry) { 
            ...
            if (!isData &&  string::npos != filen.find("ZZTo4L") )  isZZTo4L=true;
		...
		//cout<<" how many taus  "<<promptTausFirstCopy.size()<<"  mu "<<promptMuons.size()<<" electrons "<<promptElectrons.size()<<endl;

                if (isDY || isZZTo4L) {

                    if (promptTausFirstCopy.size()>=2) {
                        isZTT = true; isZMM = false; isZEE = false;
                        isDYTT=true; selEventsZTT++;
                    ...
                    if (promptMuons.size()>=2) {
                        isZTT = false; isZMM = true; isZEE = false;
                        isDYMM=true;selEventsZMM++;
            ...
            if (!isData ) {
                ...
                lumi=true;//Force true later... lumi not needed in MC
            }

            //sets lumi info and run for Data
            if (isData)  {
                XSec = 1.;
            ...
            unsigned int nMainTriggerMu = -1;
            unsigned int nMainTriggerTau = 1;
            unsigned int nMainTriggerTau2 = 1;
            ...
            for (unsigned int i=0; i<nfilters; ++i) {
                ...
                if (HLTFilter==MainTriggerMu) { nMainTriggerMu = i;isMainTriggerMu = true;}

		if (HLTFilter==MainTriggerTau) {nMainTriggerTau = i; isMainTriggerTau = true;}
                
		if (HLTFilter==MainTriggerTau2) {nMainTriggerTau2 = i; isMainTriggerTau2 = true;}

	    }

            	if (!isMainTriggerMu && !isMainTriggerTau && !isMainTriggerTau2) {
                std::cout << "HLT filters failed " << MainTriggerMu << " "<<MainTriggerTau<<"  "<<MainTriggerTau2<<" not found" << std::endl;
                return(-1);
            		}

            //      if (isSignal) isMainTriggerMu = true;

             //   std::cout << "HLT filters found " << isMainTriggerMu << " "<<isMainTriggerTau<<"  "<<isMainTriggerTau2<< std::endl;
	    ...
	    bool isMu2matched = false;
            ...
            //if (!isMu1matched) continue;//cut this for the trigger object and more tau selection ... oinclude trigger information but dont cut on it. 
            if (muons.size()==0) continue;
            //pt and mass for the di tau... run without the trigger... 
            //lower pt to be 8 
            //taus above 20GeV...
	    ...
	    //	isMu1matched = false;
          	float neutralHadIsoMu = analysisTree.muon_r04_sumNeutralHadronEt[index1];
          	float photonIsoMu = analysisTree.muon_r04_sumPhotonEt[index1];
          	float chargedHadIsoMu = analysisTree.muon_r04_sumChargedHadronPt[index1];
          	float puIsoMu = analysisTree.muon_r04_sumPUPt[index1];
        	double neutralIsoMuN = neutralHadIsoMu + photonIsoMu - 0.5*puIsoMu;
        	double neutralIsoMu = max(double(0),neutralIsoMuN);
        	float absIsoMu = chargedHadIsoMu + neutralIsoMu;
        	float relIsoMu1 = absIsoMu/analysisTree.muon_pt[index1];

		    
                    float isoMuonsVal1 = relIsoMu1;
		for (unsigned int iT=0; iT<analysisTree.trigobject_count; ++iT) {
		//	if (analysisTree.trigobject_filters[iT][nMainTriggerMu])
		//		cout<<" trigger info  "<<"  "<<analysisTree.muon_pt[index1]<<"   "<<analysisTree.trigobject_pt[iT]<<"  "<<nMainTriggerMu<<"   "<<analysisTree.trigobject_filters[iT][nMainTriggerMu]<<endl;
			if (analysisTree.trigobject_filters[iT][nMainTriggerMu]
					&& analysisTree.muon_pt[index1]>SingleMuonTriggerPtCut &&  analysisTree.trigobject_pt[iT]>SingleMuonTriggerPtCut) { // IsoMu Leg
				float dRtrig = deltaR(analysisTree.muon_eta[index1],analysisTree.muon_phi[index1],
						analysisTree.trigobject_eta[iT],analysisTree.trigobject_phi[iT]);
                                if (dRtrig<deltaRTrigMatch){
                                    isMu1matched = true;
				//cout<<" trigger info  "<<dRtrig<<"  "<<analysisTree.muon_pt[index1]<<"   "<<analysisTree.trigobject_pt[iT]<<endl;
                            	    //singleMuonMatch = isMu1matched;//trial
				}
                            }
                    }
			...
			if (int(index2) == int(index1)) continue;
			...
			if (q1*q2>0) continue;
	    		...
	    		//isMu2matched = false;
          		...
          		neutralHadIsoMu = analysisTree.muon_r04_sumNeutralHadronEt[index2];
          		photonIsoMu = analysisTree.muon_r04_sumPhotonEt[index2];
          		chargedHadIsoMu = analysisTree.muon_r04_sumChargedHadronPt[index2];
          		puIsoMu = analysisTree.muon_r04_sumPUPt[index2];
        		neutralIsoMuN = neutralHadIsoMu + photonIsoMu - 0.5*puIsoMu;
        		neutralIsoMu = max(double(0),neutralIsoMuN);
        		absIsoMu = chargedHadIsoMu + neutralIsoMu;
     
		   	float relIsoMu2 = absIsoMu/analysisTree.muon_pt[index2];

                            float isoMuonsVal2 = relIsoMu2;

                            float sumIso = isoMuonsVal1+isoMuonsVal2;
                            if (sumIso<isoMin) {
                                //cout<<" logs sumIso "<<sumIso <<" isoMin "<<isoMin<<endl; 
                                isIsoMuonsPair = true;
                                isoMin = sumIso;
...
/*
 if (indx1>-1 && indx2 >-1){
                   ...
                   TLorentzVector TVmuon = TLmu1 + TLmu2;
                    ...
                    cout<<"  mass  "<<TVmuon.M()<<"  "<<fabs (TVmuon.M() - 91)<<"  isMu1matched  "<<isMu1matched<<"  2nd muon trigger ? "<<isMu2matched<<endl;
}
*/
	    ...
	    singleMuonMatch = false;

                        isTriggerMatch = (isMu1matched || isMu2matched);
			singleMuonMatch = (isMu1matched || isMu2matched);


            ///////////loop a second time to form a 2nd mumu pair
            //
            isoMin = 9999;
            vector<int> muonsThird; muonsThird.clear();
                        
	    bool isMu3matched = false;
	    bool isMu4matched = false;

            if (isoMuons.size()>0 && indx1>-1 && indx2>-1) {
                for (unsigned int im1=0; im1<isoMuons.size(); ++im1) {
                    unsigned int index3 = isoMuons[im1];
                    if ((int) index3 == indx1 || (int) index3 == indx2) continue;


                        unsigned int indexProbe = isoMuons[index3];

                        float dRmu3mu1 = deltaR(analysisTree.muon_eta[index3],analysisTree.muon_phi[index3],
                                analysisTree.muon_eta[indx1],analysisTree.muon_phi[indx1]);
                        if (dRmu3mu1 < 0.02) continue;

                        float dRmu3mu2 = deltaR(analysisTree.muon_eta[index3],analysisTree.muon_phi[index3],
                                analysisTree.muon_eta[indx2],analysisTree.muon_phi[indx2]);
                        if (dRmu3mu2 < 0.02) continue;

                    	muonsThird.push_back((int)index3);

                        for (unsigned int iT=0; iT<analysisTree.trigobject_count; ++iT) {
                            if (analysisTree.trigobject_filters[iT][nMainTriggerMu]
                                    && analysisTree.muon_pt[index3]>SingleMuonTriggerPtCut &&  analysisTree.trigobject_pt[iT]>SingleMuonTriggerPtCut) { // IsoMu Leg
                                float dRtrig = deltaR(analysisTree.muon_eta[index3],analysisTree.muon_phi[index3],
                                        analysisTree.trigobject_eta[iT],analysisTree.trigobject_phi[iT]);
                                if (dRtrig<deltaRTrigMatch) 
                                    isMu3matched = true;
                            }
                        }
                            float absIso = analysisTree.muon_r04_sumChargedHadronPt[index3];
                            float neutralIso = analysisTree.muon_r04_sumNeutralHadronEt[index3] + 
                                analysisTree.muon_r04_sumPhotonEt[index3] - 
                                0.5*analysisTree.muon_r04_sumPUPt[index3];
                            neutralIso = TMath::Max(float(0),neutralIso); 
                            absIso += neutralIso;
                            float relIsoMu3 = absIso/analysisTree.muon_pt[index3];

                            float isoMuonsVal3 = relIsoMu3;


                    for (unsigned int im2=im1+1; im2<isoMuons.size(); ++im2) {
                        unsigned int index4 = isoMuons[im2];
                        if ((int) index4 == indx1 || (int) index4 == indx2 || (int) index4 == (int) index3) continue;
                        float q1 = analysisTree.muon_charge[index3];
                        float q2 = analysisTree.muon_charge[index4];
                        if (q1*q2>0) continue;


                        float dRmu4mu1 = deltaR(analysisTree.muon_eta[index4],analysisTree.muon_phi[index4],
                                analysisTree.muon_eta[indx1],analysisTree.muon_phi[indx1]);
                        if (dRmu4mu1 < 0.02) continue;

                        float dRmu4mu2 = deltaR(analysisTree.muon_eta[index4],analysisTree.muon_phi[index4],
                                analysisTree.muon_eta[indx2],analysisTree.muon_phi[indx2]);
                        if (dRmu4mu2 < 0.02) continue;


                        float dRmumu = deltaR(analysisTree.muon_eta[index3],analysisTree.muon_phi[index3],
                                analysisTree.muon_eta[index4],analysisTree.muon_phi[index4]);
                        ...
                        for (unsigned int iT=0; iT<analysisTree.trigobject_count; ++iT) {
                            ...
                            absIso = analysisTree.muon_r04_sumChargedHadronPt[index4];
                            neutralIso = analysisTree.muon_r04_sumNeutralHadronEt[index4] + 
                                analysisTree.muon_r04_sumPhotonEt[index4] - 
                                0.5*analysisTree.muon_r04_sumPUPt[index4];
                            neutralIso = TMath::Max(float(0),neutralIso); 
                            absIso += neutralIso;
                            float relIsoMu4 = absIso/analysisTree.muon_pt[index4];

                            float isoMuonsVal4 = relIsoMu4;


                            float sumIso = isoMuonsVal3+isoMuonsVal4;
                            if (sumIso<isoMin) {
                                //isIsoMuonsPair = true;
                                isoMin = sumIso;
                                if (analysisTree.muon_pt[index3]>analysisTree.muon_pt[index4]) {
                                    indx3 = (int)index3;
                                    indx4 = (int)index4;
                                }
                                else {
            ...
            if (indx3 >-1)
                cout<<" do I have 4 muons  ??? "<<indx1<<"  "<<indx2<<"  "<<indx3<<"  "<<indx4<<endl;


            int mu_index_1 = indx1;
            int mu_index_2 = indx2;
            int mu_index_1a = indx3;
            int mu_index_2a = indx4;

            vector<int> taus; taus.clear();

	//    cout<<" for entry  "<<iEny<<  "   "<<analysisTree.tau_count<<endl;
            for (unsigned int it = 0; it<analysisTree.tau_count; ++it) {
                ...
                if (analysisTree.tau_byVLooseIsolationMVArun2v1DBoldDMwLT[it] < 0.5) continue;

                //Assuming that particle flow orders these in pt...
	...
	if (taus.size()==0) continue;
	    ...
	    //cout<<" will look for taus now.... "<<taus.size()<<endl;


            for (unsigned int it1=0; it1<taus.size(); ++it1){
                unsigned int tIndex1 = taus.at(it1);

                //taking out the mu tau DeltaR criterion for now
                    ...
                    for (unsigned int iT1=0; iT1<analysisTree.trigobject_count; ++iT1) {
                        if ( ( analysisTree.trigobject_filters[iT1][nMainTriggerTau] || analysisTree.trigobject_filters[iT1][nMainTriggerTau2])
                                && analysisTree.tau_pt[tIndex1]>35 && 
                                analysisTree.trigobject_pt[iT1]>35) { // Trigger on above 35GeV

                            float dRtrig = deltaR(analysisTree.tau_eta[tIndex1],analysisTree.tau_phi[tIndex1],
                ...
                for (unsigned int it2=it1+1; it2<taus.size(); ++it2){
                    ...
                    unsigned int tIndex2 = taus.at(it2);//we have a second tau
                    ...
                    if (mu_index_1 >-1 && dRtau1<0.02) continue;
                    ...
                    if (mu_index_2>-1 && dRtau2<0.02) continue;

//                    if (fabs(analysisTree.tau_eta[it2])>etaTauCutTauTau) continue;
		    ...
		    if (isTauPair) selEventsTauPair++;
                    ...
                    //going to see if both taus pass the trigger ... namely 35 GeV and Delta  R cut
                        ...
                        if ( ( analysisTree.trigobject_filters[iT2][nMainTriggerTau] || analysisTree.trigobject_filters[iT2][nMainTriggerTau2])
                                    && analysisTree.tau_pt[tIndex2]>35 &&//pt cut to be 10GeV 
                                    analysisTree.trigobject_pt[iT2]>35) { // Trigger on above 35GeV

                                float dRtrig = deltaR(analysisTree.tau_eta[tIndex2],analysisTree.tau_phi[tIndex2],
...
//cout<< " story so far   "<<iEntry<< "  TauPair  "<<isTauPair<<" selEvents  "<<selEventsZTT<<" TauPair " <<selEventsTauPair<<endl;

	if (tau_index_1>-1){
                            float isoTau1 = analysisTree.tau_byIsolationMVArun2v1DBoldDMwLTraw[tau_index_1];


                            ta_IsoFlagVTight[0]=analysisTree.tau_byVTightIsolationMVArun2v1DBoldDMwLT[tau_index_1];
                            ta_IsoFlagTight[0]=analysisTree.tau_byTightIsolationMVArun2v1DBoldDMwLT[tau_index_1];
                            ta_IsoFlagMedium[0]=analysisTree.tau_byMediumIsolationMVArun2v1DBoldDMwLT[tau_index_1];
                            ta_IsoFlagLoose[0]=analysisTree.tau_byLooseIsolationMVArun2v1DBoldDMwLT[tau_index_1];
                            ta_IsoFlagVLoose[0]=analysisTree.tau_byVLooseIsolationMVArun2v1DBoldDMwLT[tau_index_1];
                            ta_relIso[0]= isoTau1;
	}
	if (tau_index_2>-1){
			    float isoTau2 = analysisTree.tau_byIsolationMVArun2v1DBoldDMwLTraw[tau_index_2];
                            ta_IsoFlagVTight[1]=analysisTree.tau_byVTightIsolationMVArun2v1DBoldDMwLT[tau_index_2];
                            ta_IsoFlagTight[1]=analysisTree.tau_byTightIsolationMVArun2v1DBoldDMwLT[tau_index_2];
                            ta_IsoFlagMedium[1]=analysisTree.tau_byMediumIsolationMVArun2v1DBoldDMwLT[tau_index_2];
                            ta_IsoFlagLoose[1]=analysisTree.tau_byLooseIsolationMVArun2v1DBoldDMwLT[tau_index_2];
                            ta_IsoFlagVLoose[1]=analysisTree.tau_byVLooseIsolationMVArun2v1DBoldDMwLT[tau_index_2];
                            ta_relIso[1]= isoTau2;
	}
	...
	//    if (!isTauPair) continue;
                ...
                float dRtau1 = deltaR(analysisTree.tau_eta[taus_index_1],analysisTree.tau_phi[taus_index_1],
                        analysisTree.muon_eta[mu_index_1],analysisTree.muon_phi[mu_index_1]);
                if (mu_index_1 >-1 && dRtau1<0.02) continue;

                float dRtau2 = deltaR(analysisTree.tau_eta[taus_index_1],analysisTree.tau_phi[taus_index_1],
                	        analysisTree.muon_eta[mu_index_2],analysisTree.muon_phi[mu_index_2]);
                if (mu_index_2 > -1 && dRtau2<0.02) continue;

                float dRtau1a = deltaR(analysisTree.tau_eta[taus_index_2],analysisTree.tau_phi[taus_index_2],
                        analysisTree.muon_eta[mu_index_1],analysisTree.muon_phi[mu_index_1]);
                if (mu_index_1>-1 && dRtau1a<0.02) continue;

                float dRtau2a = deltaR(analysisTree.tau_eta[taus_index_2],analysisTree.tau_phi[taus_index_2],
                	        analysisTree.muon_eta[mu_index_2],analysisTree.muon_phi[mu_index_2]);
                if (mu_index_2 >-1 && dRtau2a<0.02) continue;
	
		

	 //   cout<<" will look for taus now.... taus size "<<taus.size()<<" mu1 "<<mu_index_1<<" mu2  "<<mu_index_2<<" mu3 "<<mu_index_1a<<" mu4 "<<mu_index_2a<<endl;

            bool isMuonAndMuon = false;

        if (mu_index_1>-1)    mu_relIso[0]=isoMuonsValue[mu_index_1];
        if (mu_index_2>-1)    mu_relIso[1]=isoMuonsValue[mu_index_2];
        if (mu_index_1a>-1)    mu_relIso[2]=isoMuonsValue[mu_index_1a];
        if (mu_index_2a>-1)    mu_relIso[3]=isoMuonsValue[mu_index_2a];

            if (indx3 !=-1 && indx4 !=-1) isMuonAndMuon = true;
            if (isMuonAndMuon) isMuMu = true;
      ...
      //      if (event_sign >0) continue;
            ...
            // S.H. soooo what about the first Z... 
            // 1. Check other Z for taus?
            // 2. Check that the cuts match Run2...
            // 3. Is this on-shell off shell Z ... how do I even check this?
	...
	/*
        for (unsigned int im=0; im<muonsThird.size(); ++im) {
            ...
            float absIso = analysisTree.muon_r04_sumChargedHadronPt[mIndex];
            float neutralIso = analysisTree.muon_r04_sumNeutralHadronEt[mIndex] + 
                analysisTree.muon_r04_sumPhotonEt[mIndex] - 
                0.5*analysisTree.muon_r04_sumPUPt[mIndex];
            neutralIso = TMath::Max(float(0),neutralIso); 
            absIso += neutralIso;
            float relIsoMu = absIso/analysisTree.muon_pt[mIndex];
...
*/
  ...
  //              if (int(ie)==(int)el_index) continue;
            ...
            for (unsigned int im = 0; im<analysisTree.muon_count; ++im) {
                ...
                if (int(im)==(int)mu_index_1a) continue;
                if (int(im)==(int)mu_index_2a) continue;
                if (isData && analysisTree.muon_isDuplicate[im]) continue;
                if (isData && analysisTree.muon_isBad[im]) continue;
            ...
            if (!isData && applyLeptonSF) {
                ...
                float IdIsoSF_mu2 = 1;//SF_muonIdIso->get_ScaleFactor(ptMu2, etaMu2);
                if (mu_index_2>-1) SF_muonIdIso->get_ScaleFactor(ptMu2, etaMu2);
            ...
            muon_index_1a = (int)mu_index_1a;
            muon_index_2a = (int)mu_index_2a;
            muon_index_3 = (int)mu_index_3;
            electron_index = (int)el_index;
            //taus_index carries the index of the lepton in the case of MuTau or ElTau
            ...
            for (unsigned int jet=0; jet<analysisTree.pfjet_count; ++jet) {
                ...
                if (  mu_index_1>-1 && Dr  < DRmax)  cleanedJet=false;


                double Drr=deltaR(analysisTree.muon_eta[mu_index_2],analysisTree.muon_phi[mu_index_2],
                        analysisTree.pfjet_eta[jet],analysisTree.pfjet_phi[jet]);

                if ( mu_index_2>-1 && Drr < DRmax) cleanedJet=false;
...
*/
  //              if (isTauTau ) {
                ...
                if ( Drrt3 < DRmax) cleanedJet=false;
    ...
    std::cout << "Total number of selected events = " << selEvents << "  Z->Mumu "<<selEventsZMM<<" Z->elel  "<<selEventsZEE<<"  Z->tautau "<<selEventsZTT<<"  and Tau pair  "<<selEventsTauPair<<std::endl;
    std::cout << std::endl;
